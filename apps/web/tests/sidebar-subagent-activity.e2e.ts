...
import type { AgentHandle } from '@monotykamary/dsh-agent'
import { createUserMessage, LlmAdapter } from '@monotykamary/dsh-llm'
import type { GenerateOptions, StreamChunk } from '@monotykamary/dsh-llm'
import { SessionId, type SessionId as SessionIdValue } from '@monotykamary/dsh-session'
import type {} from '@monotykamary/dsh-subagent'
...
describe('web e2e: sidebar subagent activity', () => {
  ...
  it('pins a running descendant on its visible idle owner row', async () => {
    ...
    const ownerRow = sidebar.locator('[role="treeitem"]')
      .filter({ hasText: 'Delegate a background job' })
      .filter({ has: page.locator('[data-state="ongoing"]') })
