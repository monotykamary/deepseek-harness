...
// credential is absent, the credential step owns the shipped modal chrome,
...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  it('never paints the takeover chrome on a configured reload, even with the settings join held open', async () => {
    ...
    // yet it must load private facts before deciding not to show. Dialog
    ...
    // loopback RTT scale to a deterministic hundreds of milliseconds,
  ...
  it('configures arbitrary DeepSeek models and prompts after the selected model is removed', async () => {
    ...
    // Exact-model capabilities and capacities share the row's details disclosure.
    await settings.getByRole('button', { name: '模型详情 3' }).click()
    await settings.getByLabel('接受图片输入 3').check()
