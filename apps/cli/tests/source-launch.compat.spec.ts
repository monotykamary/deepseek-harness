 ...
 * with the exact production runtime vector (`node --expose-internals --import tsx/esm`, the
 ...
 * breaks this gate instead of every developer's `Bun dsh`; the built-bin
...
describe('dsh SOURCE launcher (node --expose-internals --import tsx/esm)', () => {
  ...
  it('boots the source entry and requires a profile', async () => {
    const result = await execa(process.execPath, ['--expose-internals', '--import', 'tsx/esm', dshSourceBin], {
      cwd: repoRoot,
      input: '',
      timeout: 25_000,
      killSignal: 'SIGKILL',
