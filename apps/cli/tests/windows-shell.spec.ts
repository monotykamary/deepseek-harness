...
import { entryListSchema } from '@monotykamary/cordis-plugin-include'
import { evaluate } from '@monotykamary/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@monotykamary/dsh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@monotykamary/dsh-base', '@monotykamary/dsh-web-app'])
    ...
    for (const name of ['@monotykamary/dsh-pwsh-sandbox', '@monotykamary/dsh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@monotykamary/dsh-base'])
