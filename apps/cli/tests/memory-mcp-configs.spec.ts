...
import { boot, loadOverlayPatches } from '@monotykamary/dsh-app-boot'
import SystemPrompt from '@monotykamary/dsh-system-prompt'
import ToolRuntime from '@monotykamary/dsh-tools'
import * as McpClient from '@monotykamary/dsh-mcp-client/src/index.ts'
...
describe('third-party memory MCP example overlays', () => {
  it.each(examples)('parses $file with the documented generic plugin fields', (contract) => {
    ...
    expect(row.name).toBe('@monotykamary/dsh-mcp-client')
