...
import { startMockLlmServer } from '@monotykamary/dsh-llm-mock-server'
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    dsh: { profile: { bundles: ['@monotykamary/dsh-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@monotykamary/dsh-cmdline` and `commander` through the profile module
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@monotykamary/dsh-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @monotykamary/dsh-base')
      expect(stdout).toContain("name: '@monotykamary/dsh-host-webserver'")
    }, 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@monotykamary/dsh-headless'")
      expect(stdout).not.toMatch(/name: '@monotykamary\/dsh-host-/)
      expect(stdout).not.toContain("name: '@monotykamary/dsh-web-app'")
      expect(stdout).not.toMatch(/name: '@monotykamary\/dsh-client-/)
    }, 30_000)
