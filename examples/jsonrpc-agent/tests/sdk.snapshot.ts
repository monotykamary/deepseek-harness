 ...
 * REAL `@monotykamary/dsh-sdk-client`, drives one turn over stdio JSON-RPC,
...
import { resolveExampleLaunch } from '@monotykamary/dsh-loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@monotykamary/dsh-sdk-client'
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@monotykamary/dsh-system-prompt') return []
