...
/** Drop the `@monotykamary/dsh-` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@monotykamary/dsh-', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @monotykamary/dsh-cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
