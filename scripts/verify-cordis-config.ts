...
const CHOOSER_PACKAGE = '@monotykamary/dsh-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@monotykamary/dsh-host-directory-picker-native',
  '@monotykamary/dsh-host-directory-picker-browse',
  '@monotykamary/dsh-client-ui-directory-picker-browse',
  '@monotykamary/dsh-client-ui-directory-picker-native',
]
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if ((value.group === true || value.name === '@monotykamary/cordis-plugin-group') && isUnknownArray(value.config)) {
  ...
  if (value.name !== '@monotykamary/cordis-plugin-include') return
