/**
 * Rescope the vendored Cordis packages into the `@monotykamary` scope, and undo
 ...
 * `@monotykamary/dsh-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@monotykamary/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@monotykamary/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@monotykamary/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@monotykamary/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@monotykamary/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@monotykamary/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@monotykamary/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@monotykamary/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@monotykamary/cordis-plugin-logger-console' },
]
...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  // `cordis/*` event keys, the `cordis` locale namespace, and the `cordis`
  // plugin-inventory name are harness product vocabulary shared verbatim
  // between host and client (remote-events.ts forwards the tuples over the
  // gateway wire), not references to the vendored cordis package.
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@monotykamary/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@monotykamary/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@monotykamary\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@monotykamary/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@monotykamary/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@monotykamary/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@monotykamary/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@monotykamary/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@monotykamary/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@monotykamary` rescope**', count: 1 },
  { file: 'knip.json', text: '@cordisjs', count: 0 },
  { file: 'bunfig.toml', text: '"cordis",', count: 0 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@monotykamary\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@monotykamary/cordis']
    const dev = manifest.devDependencies?.['@monotykamary/cordis']

    if (!peer) errors.push(\`\${label}: @monotykamary/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @monotykamary/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @monotykamary/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
    id: 'knip-bundle-base',
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@monotykamary/\')) {',
    replace: `      // Vendored packages are rescoped too (vendor/README.md), so publication
      ...
      if (!name.startsWith('@monotykamary/')) {`,
    expect: 1,
  },
  {
    id: 'vendor-readme-preamble',
    file: 'vendor/README.md',
    find: 'All vendored packages keep their **original npm names** and are marked `private: true` — they are never published from this repo. Bun’s root `package.json#workspaces` makes matching upstream semver ranges resolve these pinned workspaces, including imports from built `lib/`; removing a member substitutes npm copies behind the same names.',
    replace: 'All vendored packages are **renamed into the `@monotykamary` scope** (`cordis` → `@monotykamary/cordis`, `@cordisjs/plugin-<x>` → `@monotykamary/cordis-plugin-<x>`): every harness package declares `cordis` as a peer dependency, so publishing the harness publishes this framework layer too, and a publication under the upstream names would squat them on the registry. Directory names and upstream version numbers are deliberately unchanged, so the manifest below still reads as an upstream snapshot. Bun’s root `package.json#workspaces` resolves those preserved semver ranges to these pinned workspaces, including imports from built `lib/`.',
    expect: 1,
  },
  {
    ...
    replace: 'whose lazy `require(\'@monotykamary/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@monotykamary/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@monotykamary/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @monotykamary, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@monotykamary\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@monotykamary/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@monotykamary\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@monotykamary/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
  ...
  // rescoped into @monotykamary, so the consumer installs this repository's
...
function excluded(file: string): boolean {
  ...
  if (file === 'bun.lock' || file === 'pnpm-lock.yaml') return true // package-manager locks are regenerated
