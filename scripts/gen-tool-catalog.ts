...
import SqliteSessionQueryEngine from '@monotykamary/dsh-session-query-sqlite'
import GoalService from '@monotykamary/dsh-goal'
import SystemPrompt from '@monotykamary/dsh-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@monotykamary/dsh-tools'
import LocalBashExecutor from '@monotykamary/dsh-bash-local'
import * as BashEnvPlugin from '@monotykamary/dsh-shell-env'
import { PwshLocalExecutor } from '@monotykamary/dsh-pwsh-local'
...
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@monotykamary/dsh-attachment'
import UserQuestionService from '@monotykamary/dsh-user-questions'
import PlanModeController from '@monotykamary/dsh-plan-mode'
import WebRuntime from '@monotykamary/dsh-web'
import * as WebSearchExa from '@monotykamary/dsh-web-search-exa'
import * as WebFetchLocal from '@monotykamary/dsh-web-fetch-http'
import SubagentRuntime from '@monotykamary/dsh-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@monotykamary/dsh-subagent'
import * as ToolSubagentControl from '@monotykamary/dsh-tool-subagent-control'
import * as ToolSubagentListAgents from '@monotykamary/dsh-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@monotykamary/dsh-tool-subagent-report'
import SkillRegistry from '@monotykamary/dsh-skill'
import * as SkillFileSystem from '@monotykamary/dsh-skill-filesystem'
import LocalJobRegistry from '@monotykamary/dsh-jobs-local'
import * as ToolAskUser from '@monotykamary/dsh-tool-ask-user'
import * as ToolBash from '@monotykamary/dsh-tool-bash'
import * as ToolPwsh from '@monotykamary/dsh-tool-pwsh'
import * as ToolBashPersistent from '@monotykamary/dsh-tool-bash-persistent'
import * as ToolPwshPersistent from '@monotykamary/dsh-tool-pwsh-persistent'
import CordisHostRunner from '@monotykamary/dsh-cordis-host-runner'
import * as ToolCordis from '@monotykamary/dsh-tool-cordis'
import * as ToolFs from '@monotykamary/dsh-tool-fs'
import * as ToolFsSearch from '@monotykamary/dsh-tool-fs-search'
import * as ToolStrReplaceEditor from '@monotykamary/dsh-tool-str-replace-editor'
import TerminalSessionService from '@monotykamary/dsh-terminal'
import * as ToolPty from '@monotykamary/dsh-tool-terminal'
import * as ToolGoal from '@monotykamary/dsh-tool-goal'
import * as ToolSchedule from '@monotykamary/dsh-schedule'
import Lsp from '@monotykamary/dsh-lsp'
import * as ToolLsp from '@monotykamary/dsh-tool-lsp'
import * as ToolSkill from '@monotykamary/dsh-tool-skill'
import * as ToolSessionQuery from '@monotykamary/dsh-tool-session-query'
import * as ToolTasks from '@monotykamary/dsh-tool-jobs'
...
import * as ToolSessionMutations from '@monotykamary/dsh-tool-session-mutations'
...
import * as ToolRalph from '@monotykamary/dsh-tool-ralph'
import * as ToolWorkflow from '@monotykamary/dsh-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@monotykamary/dsh-tool-ask-user',
    dir: 'tool-ask-user',
  ...
  {
    pkg: '@monotykamary/dsh-plan-mode',
    dir: 'plan-mode',
  ...
  {
    pkg: '@monotykamary/dsh-tool-bash',
    dir: 'tool-bash',
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@monotykamary/dsh-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@monotykamary/dsh-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@monotykamary/dsh-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `DSH_*` environment comes from `@monotykamary/dsh-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@monotykamary/dsh-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@monotykamary/dsh-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@monotykamary/dsh-tool-bash-persistent',
  ...
  {
    pkg: '@monotykamary/dsh-tool-pwsh-persistent',
  ...
  {
    pkg: '@monotykamary/dsh-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@monotykamary/dsh-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. `read_image` is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@monotykamary/dsh-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  {
    pkg: '@monotykamary/dsh-tool-terminal',
    dir: 'tool-terminal',
  ...
  {
    pkg: '@monotykamary/dsh-tool-goal',
    dir: 'tool-goal',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@monotykamary/dsh-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@monotykamary/dsh-tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@monotykamary/dsh-tool-skill',
    dir: 'tool-skill',
  ...
  {
    pkg: '@monotykamary/dsh-tool-session-query',
  ...
  {
    pkg: '@monotykamary/dsh-tool-subagent-report',
  ...
  {
    pkg: '@monotykamary/dsh-tool-jobs',
    dir: 'tool-jobs',
  ...
  {
    pkg: '@monotykamary/dsh-experimental-tool-agent-team',
    dir: 'tool-agent-team',
  ...
  {
    pkg: '@monotykamary/dsh-tool-session-mutations',
    dir: 'tool-session-mutations',
    source: 'packages/session-query/tool-session-mutations/src/index.ts',
    requires: ['ctx.tools', 'owning Agent session'],
    writes: ['tool/call', 'tool/result'],
    async mount(ctx) {
      await ctx.plugin(ToolSessionMutations, { maxListItems: 50, maxDiffChars: 12_000 })
    },
  ...
  {
    pkg: '@monotykamary/dsh-tool-workflow',
    dir: 'tool-workflow',
