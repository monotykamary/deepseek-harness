...
const CORDIS = '@monotykamary/cordis'
const roots: string[] = []
...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@monotykamary/dsh-client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@monotykamary/dsh-a/subpath'",
      "declare module '@monotykamary/dsh-client-ui-slots' {}",
      "const load = () => import('@monotykamary/dsh-b')",
...
describe('package modes', () => {
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@monotykamary/dsh-client-runtime/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('dependency sections', () => {
  it('accepts dynamic peer plus dev relationships, static dev inputs, and private dependencies', () => {
    ...
    const runtime = pkg('runtime', {
      inject: ['@monotykamary/dsh-client-feature'],
      sourceUses: {
        '@monotykamary/dsh-agent': ['packages/client/runtime/src/index.ts'],
        '@monotykamary/dsh-client-ui-slots': ['packages/client/runtime/src/client/slots.ts'],
        react: ['packages/client/runtime/src/client/view.tsx'],
      },
      dependencies: { immer: '^10.1.1' },
      peerDependencies: {
        [CORDIS]: 'workspace:^',
        '@monotykamary/dsh-agent': 'workspace:^',
        '@monotykamary/dsh-client-feature': 'workspace:^',
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@monotykamary/dsh-agent': 'workspace:^',
        '@monotykamary/dsh-client-feature': 'workspace:^',
        '@monotykamary/dsh-client-ui-slots': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('rejects internal dependencies, static peers, and mismatched peer development ranges', () => {
    ...
    const subject = pkg('feature', {
      sourceUses: {
        '@monotykamary/dsh-agent': ['packages/client/feature/src/index.ts'],
        [slots.name]: ['packages/client/feature/src/view.tsx'],
      },
      dependencies: { '@monotykamary/dsh-agent': 'workspace:^' },
  ...
  it('requires every peer to have the same development range', () => {
    const subject = pkg('feature', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@monotykamary/cordis-plugin-loader': 'workspace:^' },
    })
    expect(collectClientPackageViolations(facts([subject]))).toEqual([
      'packages/client/feature/package.json: peerDependencies.@monotykamary/cordis-plugin-loader'
      + ' is workspace:^, so devDependencies.@monotykamary/cordis-plugin-loader must use the same range;'
  ...
  it('keeps the web shell runtime inputs development-only', () => {
    const web = pkg('web', {
      ...
      staticLinked: true,
      runtimeSourceUses: {
        '@monotykamary/cordis-plugin-loader': ['packages/client/web/src/boot.ts'],
        react: ['packages/client/web/src/seed.ts'],
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@monotykamary/cordis-plugin-loader': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('allows npm dependency cycles', () => {
    const a = pkg('a', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@monotykamary/dsh-client-b': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@monotykamary/dsh-client-b': 'workspace:^' },
    })
    const b = pkg('b', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@monotykamary/dsh-client-a': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@monotykamary/dsh-client-a': 'workspace:^' },
    })
...
describe('module requests', () => {
  it('accepts a dynamic row supplier and its client subpath', () => {
    const ui = declaration('ui', { external: ['@monotykamary/dsh-client-slots/client'] })
  ...
  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@monotykamary/dsh-client-ui', '@monotykamary/dsh-missing', '@monotykamary/dsh-missing'],
      inject: ['', '@monotykamary/dsh-a', '@monotykamary/dsh-a'],
    })
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = declaration('a', {
      external: ['@monotykamary/dsh-client-b'],
      inject: ['@monotykamary/dsh-client-b'],
    })
    const b = declaration('b', {
      external: ['@monotykamary/dsh-client-a'],
      inject: ['@monotykamary/dsh-client-a'],
    })
...
describe('manifest declarations', () => {
  ...
  it('fixes unambiguous dependency sections and declaration entries', () => {
    ...
    roots.push(root)
    const subject = pkg('feature', {
      external: ['', 'react', '@monotykamary/dsh-client-feature', '@monotykamary/dsh-missing'],
      inject: ['', '@monotykamary/dsh-agent', '@monotykamary/dsh-agent'],
      sourceUses: {
        '@monotykamary/dsh-agent': ['packages/client/feature/src/index.ts'],
        '@monotykamary/dsh-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@monotykamary/dsh-agent': 'workspace:*',
    ...
    expect(fixed.dsh.client).toMatchObject({
      external: ['@monotykamary/dsh-missing'],
      inject: ['@monotykamary/dsh-agent'],
    })
