...
const SESSION_PACKAGE = '@monotykamary/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@monotykamary/dsh-session/types'
 ...
 * top-level declaration (in `@monotykamary/dsh-session`) and any declaration
 * merge inside a `declare module '@monotykamary/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @monotykamary/dsh-session/known-event-types',
    ' */',
    '',
    '/**',
