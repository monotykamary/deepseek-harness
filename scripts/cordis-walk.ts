...
const MERGE_HEAD = /declare module ['"](?:@monotykamary\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@monotykamary/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@monotykamary/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@monotykamary/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@monotykamary/cordis'` Context merge
...
/**
 * Every event name a `declare module '@monotykamary/cordis'` Events merge declares in one
