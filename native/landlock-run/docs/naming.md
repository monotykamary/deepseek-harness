...
The public package family belongs to the `@monotykamary` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@monotykamary/node-addon-landlock-run
@monotykamary/node-addon-landlock-run-<platform>
```
