...
**设置外壳协调有序步骤。** `settings.onboarding` 仍是根作用域 list，但 `ui-settings` 会把其中各条目的 id 和顺序投影到一个协调器中，并且只挂载第一个未完成的步骤。当前注册方会收到 `complete()` 和 `openSection(id)`；所有权转移前，不会挂载后续步骤。`ui-settings-models` 以顺序 `0` 注册 DeepSeek 条件式凭据步骤，并曾以顺序 `-100` 注册欢迎声明，直到[移除内部测试声明](../simplification/2026-08-18-remove-testing-stage-welcome-notice.md)。共用弹窗展示由[共用弹窗引导决策](2026-08-13-shared-modal-product-onboarding.md)持有。

**产品欢迎步骤按版本管理并归功能插件所有。** 该声明曾由[移除首次启动内测声明](../simplification/2026-08-13-remove-first-run-beta-notice.md)历史决策移除，以新的测试阶段文案恢复在 `ui-settings-models` 中，又由[移除内部测试声明](../simplification/2026-08-18-remove-testing-stage-welcome-notice.md)再次移除。`ui-settings-general` 不注册任何引导步骤；持有 DeepSeek 步骤的插件也持有共用弹窗。
...
全新 profile 现在直接进入条件式 DeepSeek 密钥弹窗（当没有任何可用提供方时）；其前的测试阶段声明已由[移除内部测试声明](../simplification/2026-08-18-remove-testing-stage-welcome-notice.md)移除。定向 store 与 React 测试固定协调器顺序、条件式移交、共用弹窗行为与 HMR 清理。真实 Chromium 场景会在隔离的 harness 家目录下启动已发布 Web 组合，验证凭据弹窗，通过既有凭据边界写入密钥，并检查 secret 未进入 DOM、ARIA 或浏览器控制台。
