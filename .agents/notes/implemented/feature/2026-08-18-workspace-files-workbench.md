...
Host tests pin direct Remote registration, provider-order projection, escaped-target suppression, traversal depth, configurable read/write caps, complete UTF-8 reads, guarded provider write intents, saved versions, conflicts, unavailable reasons, cancellation, and loud missing-authority failures. Client tests pin per-Session cache and cancellation behavior, loaded-node filtering, keyboard tree navigation, SourceEditor editing, wrap, and fullscreen behavior, single-flight autosave, all save outcomes, and presentation/header registration. The keyless Web journey boots the shipped composition; opens, collapses, and reopens the empty panel through one header toggle; lists and expands a real temporary workspace through the Remote; expands and restores the syntax-highlighted editor; edits source; observes the saved value on disk; filters loaded rows; verifies tab icon/close swapping; and retains inline and compact hosting.
