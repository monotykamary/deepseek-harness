...
- The privileged /api plane follows the same live trusted-authority list as the ordinary fence, so a tailnet or portless authority reaches the configuration plane too; the grant's edges come from the [web identity layer](2026-08-17-web-identity-sso.md) and the [privileged-plane trusted surfaces note](../architecture/2026-08-18-privileged-plane-trusted-surfaces.md): the operator bearer token works from any surface, and a partitioned user reaches ordinary RPC only.
