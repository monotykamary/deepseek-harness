...
Host 测试固定 direct Remote 注册、provider 顺序投影、越界 target 隐藏、遍历深度、可配置读写上限、完整 UTF-8 读取、带防护的 provider 写入 intent、保存版本、冲突、不可用原因、取消与缺失权威时的明确失败。Client 测试固定每 Session 缓存与取消行为、已加载节点筛选、文件树键盘导航、SourceEditor 编辑、换行与全屏行为、单次在途自动保存、全部保存结果及展示／标题栏注册。无密钥 Web journey 启动发货组合；通过同一个标题栏切换控件打开、收起并重新打开空面板；经 Remote 列出并展开真实临时工作区；展开并恢复带语法高亮的编辑器；编辑源码；在磁盘上观测保存值；筛选已加载行；验证标签页图标／关闭按钮切换；并保留内联与紧凑承载。
