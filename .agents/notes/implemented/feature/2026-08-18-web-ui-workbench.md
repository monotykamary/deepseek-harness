...
`ui-workbench` occupies the layout-owned `details` slot and declares the session-scoped `workbench.surface` list. Each feature registers its own list entry with a stable id and locale-following label plus effect-scoped icon and launcher-description metadata. The workbench projects those registrations into available surfaces, keeps open and active branded ids in a per-session entry store, and exposes `ctx.workbench.show()` for an empty panel plus `open(sessionId, id)`, `openNew(sessionId, id)`, and `ensureCount(sessionId, id, count)` for stateful feature gestures. Each mounted Details component binds its actions under its exact Session id and retracts that binding on unmount; the service resolves only the named Session, then rejects an unavailable binding or unregistered id before changing layout state.

An empty panel remains open and presents every registered surface as a centered launcher card. Closing the final tab returns to that launcher instead of inferring that the user also intended to close the panel. Tabs reserve one leading icon seat; hover or keyboard focus replaces the icon with the close glyph in place, so labels never shift. An immersive surface owns the top chrome only while it is the sole open surface; opening another restores generic tabs and cross-surface navigation. `ui-conversation` contributes the right-panel glyph to its right-aligned Session utility list and routes it through `show()`, while feature-specific actions remain direct `open(sessionId, id)` shortcuts.
...
Per-file coverage pins the workbench store, presentation projection, explicit two-Session service routing and binding disposal, singleton reuse, empty launcher, icon/close tabs, registration teardown, right-panel utility, responsive Sheet, deliverables Definition, incremental target, and the Changes presenter’s independent and all-row disclosure states. Keyless Web journeys boot the shipped Loader composition, open the empty panel from its header glyph, launch Files from a card, inspect tab hover behavior, and verify inline and compact hosting alongside the recorded mutation path through Changes and Inspect.
