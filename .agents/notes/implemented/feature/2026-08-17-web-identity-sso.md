...
- **Privileged plane** — the operator tier (owner `null`) IS the privileged plane. Authenticated non-operator users reach ordinary RPC only, from any origin; the `PRIVILEGED_METHODS` set additionally requires owner `null` AND (loopback, or admission through the operator bearer token). A partitioned user is refused privileged methods even on loopback — in passkey mode the token is how the operator works.
...
- The earlier surface-resolution note's consequence "the privileged /api plane stays loopback-pinned" is superseded by this layer; that note now cross-links here.
