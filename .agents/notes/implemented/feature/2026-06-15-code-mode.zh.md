...
2. **代码执行是一个能力 seam**——`packages/code-runtime/` 包含 Service Definition 包 `@monotykamary/dsh-code-runtime`，拥有 `ctx.codeRuntime`（[能力 seam](../architecture/2026-06-13-capability-seams.zh.md)；消费方 = `dsh-tools`，core 消费 seam 的先例见 `agent-loop` → `dsh-llm`）。运行时对工具一无所知：它接收一段程序和命名的异步绑定，执行程序，报告 `{ value, logs, error? }`。语言和基底是后端属性，因此未来的 Python 或容器后端只是另一个 Service Provider 包，而非重新设计。
3. **交付的实现是 `@monotykamary/dsh-code-runtime-worker-thread`**：每次运行 spawn 一个全新的 Node worker 线程，对模型的 TypeScript 进行 type-strip 后执行，绑定通过消息端口桥接，环境为空，堆/输出/时间上限可配置，并支持硬终止。其信任姿态在设计上等同于 bash——无需 unsafe-acknowledgement flag——因为 harness 已经交付了 `dsh-bash-local`，后者以严格*更高*的环境权限执行模型编写的任意 shell 命令。
...
在 `'code'` 和 `'both'` 下，注册表拥有 `run_code` 作为保留的呈现传输通道。默认的 `required` 标签策略接受 `{ code: string; display: { name: string; description?: string } | string }`；字符串分支是简洁名称的简写。进程配置或作用域呈现可选择 `inferred`，把 `display` 变为可选，并根据已记录 `code` 中的字面量工具调用确定性派生缺失的 UI 与压缩标题（最终回退为 `Run code`）。在两种策略下，非空的 `display.name` 都优先作为标题，非空的 `display.description` 则作为独立的执行目标保留，供进度界面与持久投影使用。使用旧扁平 `description` 形态的日志调用仍可兼容回放，并将其作为旧式标题回退。它由一个正常的 `ToolDefinition` 表示以供分发，但位于可过滤的能力层之外，因此限制规则不会意外移除 Code Mode 的唯一入口。调用遍历完整的工具流水线——`tools/pre-execute` → 单调性守卫 → `tools/execute` 包裹分发 → `tools/post-execute` → 由定义拥有的可选 `finalizeContent` → 不可变的 `tools/result` 通知——与原生调用完全一致；权限插件可以在程序运行前检查程序文本，最终结果观察者看到的是规范化的外层结果。其 `execute(args, exec)`：

1. **构建绑定。** 一个 run 级别的 signal 跟随外层取消，并在 run 结算时被 abort。每个可见工具绑定都会对无损 JSON 参数创建快照，进入原生约定的分发池（调度设计由[实时并行 Agent Note](2026-07-26-code-mode-live-parallel-dispatch.zh.md) 负责），以确定性的 call id 和外层 token 作为 `parent` 执行，通过外层 execution 延后返回的上下文，并记录 `tool/code-dispatch-start`/`tool/code-dispatch` 事件对，其中结算侧携带完整渲染后的结果内容。成功时返回工具最终的规范 JSON 值；失败则变为程序可见的 `ToolCallError`。每个子调用保留自己不可变的执行标识，并遍历完整的工具流水线。绑定命名空间还保留 `describe` 与 `call`：`describe` 读取执行 agent 在运行开始时快照的目录并返回分离的 schema，`call` 则让动态工具名称经过与直接成员相同的嵌套调度器和实时作用域解析。
...
**呈现。** `run_code` 的 render intent 按[呈现意图 Agent Note](../architecture/2026-07-02-tool-render-intent-union.zh.md)在此决定：`presentCall` 创建一个 `generic` 卡片，`kind: 'execute'`，按顺序以显式非空 display 名称（或字符串简写）、确定性的词法回退或旧扁平 description 回退作为标题；非空的 `display.description` 成为卡片 `content`，程序文本仍作为 `rawInput`。这样既把紧凑的活动标签与目标分离，也使两者都能从日志参数回放。`run_code` 有意不声明 `presentResult`，因此 TUI 和宿主/客户端运行时（Web）会通过通用原始内容回退机制，使用最终持久化的 `tool/result.content` 补全该卡片，其中包括捕获的日志，以及返回值、失败信息或 post-policy spill 预览。这不是 `terminal` 卡片：该卡片的语义是「工作目录中的 shell 命令」，程序不是。参见[结果卡片完整性说明](../../archived/bug-fix/2026-07-20-code-mode-result-card-completeness.md)。
...
`packages/code-runtime/code-runtime/`——`@monotykamary/dsh-code-runtime`，仅依赖 `cordis`。一个抽象的 `CodeRuntime extends Service`（`super(ctx, 'codeRuntime')`）加上词汇：
...
`@monotykamary/dsh-code-runtime-worker-thread`，`packages/code-runtime/` 组的第二个包。每次 `run()`：
...
SDK 指示模型编写一个所加载运行时语言的异步函数体（默认可擦除 TypeScript；Python 运行时下为 Python `async` 函数体——见[语言分发 note](2026-07-31-code-mode-language-dispatch.zh.md)），通过 `await tools.name(args)` 调用已声明工具，用 `tools.describe(name)` 发现被省略的能力，再用 `tools.call({ name, args })` 分发它，在需要时捕获被拒绝的调用，并仅 return 或 log 应重新进入上下文的输出。两种 flavor 用各自的原语陈述同一约定：相互独立的只读调用可以（MAY）在 `Promise.all`（TypeScript）或 `asyncio.gather`（Python）下重叠，有副作用的调用按提交顺序单独运行，有依赖的工作用 `await` 排序。声明前缀可能与原生 schema 一样大，尤其在 `'both'` 下，但对提供方缓存保持稳定。

默认传输说明与两种 flavor 的 SDK 说明都以点名 `code` 和 `display` 这两个必填参数开头。它们将 `display` 描述为 `{ name, description? }`：`name` 是简洁的活动标题，`description` 是可选的执行目标；协议 schema 还接受非空名称字符串简写。进程级或作用域 `runCodeLabel: inferred` 声明会同步改变 schema 与对应语言的调用说明：此时只有 `code` 必填，省略或留空的 display 名称使用同一个纯标题解析器，持久投影也可对日志参数应用该解析器。无论标题是显式提供还是推导而来，提供的目标都保持独立。
...
- **注册表集成：** 测试覆盖代码生成、严格与推导式运行标签策略、所有呈现模式、保留名称和限制规则、scoped 可见性与运行作用域发现助手、权威组装重写、`toolOrder`、运行时兼容性失败、完整流水线子分发、parent-token 关联、序列化、取消和队列排空、JSON 规范化、错误传播、日志事件、成功与失败程序中的有序上下文延后、外层阻止抑制以及 HMR（热模块替换）清理。
- **带密钥 e2e：** 真实模型在一个程序中组合两次 bash 调用；另一个模型通过 Code Mode fs 分发发现嵌套的工作区指令。测试验证折叠的请求头、关联的分发事件、结果文件、延后上下文和模型行为。
- **快照：** `code-mode-turn`、`code-mode-inferred-label`、`both-mode-turn` 和 `code-mode-workspace-context` fixture（测试前置数据）固定严格与可选 schema、与策略一致的 SDK 文本、无 description 执行、请求头工具列表、分发事件、延后上下文和结果卡片。
...
**用一个扁平 `description` 字段同时承担标题与目标。** 否决，因为进度界面需要稳定的短活动名称，而确定性压缩与拓扑受益于更完整的目标。让一个字符串承担两种角色，会迫使调用方要么截断意图，要么让每张卡片的标题都变成长句。结构化 `display` 信封与 Fabric 的代码执行约定一致，明确区分两种角色，同时为只需要名称的调用方保留字符串简写；旧日志中的扁平 description 继续兼容回放，而不必留在面向模型的 schema 中。
