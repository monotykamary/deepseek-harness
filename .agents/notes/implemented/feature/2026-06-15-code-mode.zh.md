...
在 `'code'` 和 `'both'` 下，注册表拥有 `run_code` 作为保留的呈现传输通道。默认的 `required` 标签策略接受 `{ code: string; description: string }`；进程配置或作用域呈现可选择 `inferred`，把 `description` 变为可选，并根据已记录 `code` 中的字面量工具调用确定性派生 UI 与压缩标题（最终回退为 `Run code`）。它由一个正常的 `ToolDefinition` 表示以供分发，但位于可过滤的能力层之外，因此限制规则不会意外移除 Code Mode 的唯一入口。调用遍历完整的工具流水线——`tools/pre-execute` → 单调性守卫 → `tools/execute` 包裹分发 → `tools/post-execute` → 由定义拥有的可选 `finalizeContent` → 不可变的 `tools/result` 通知——与原生调用完全一致；权限插件可以在程序运行前检查程序文本，最终结果观察者看到的是规范化的外层结果。其 `execute(args, exec)`：
...
**呈现。** `run_code` 的 render intent 按[呈现意图 Agent Note](../architecture/2026-07-02-tool-render-intent-union.zh.md)在此决定：`presentCall` 创建一个 `generic` 卡片，`kind: 'execute'`，以显式非空 description 或确定性的词法回退作为标题，并将程序文本作为 `rawInput`；`run_code` 有意不声明 `presentResult`，因此 TUI 和宿主/客户端运行时（Web）会通过通用原始内容回退机制，使用最终持久化的 `tool/result.content` 补全该卡片，其中包括捕获的日志，以及返回值、失败信息或 post-policy spill 预览。这不是 `terminal` 卡片：该卡片的语义是「工作目录中的 shell 命令」，程序不是。参见[结果卡片完整性说明](../../archived/bug-fix/2026-07-20-code-mode-result-card-completeness.md)。
...
默认传输说明与两种 flavor 的 SDK 说明都以点名 `code` 和 `description` 这两个必填参数开头。进程级或作用域 `runCodeLabel: inferred` 声明会同步改变 schema 与对应语言的调用说明：此时只有 `code` 必填，省略或留空的 `description` 使用同一个纯标题解析器，持久投影也可对日志参数应用该解析器。
