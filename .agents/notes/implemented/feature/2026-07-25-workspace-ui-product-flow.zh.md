...
- 当前空白 Session 可显示为唯一的 New Session 行，同时不暴露其他可复用空白会话，也不显示 Session 数量。列表表面已由 [New Session 临时页面修复](../bug-fix/2026-08-20-new-session-ephemeral-until-first-prompt.zh.md) 取代：空白 Session 在首条提示词转正之前不渲染任何行。
