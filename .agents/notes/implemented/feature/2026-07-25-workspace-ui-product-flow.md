...
- The current blank Session can appear as a single New Session row without exposing other reusable blanks or a Session count. Superseded for list surfaces by the [New Session ephemeral-page fix](../bug-fix/2026-08-20-new-session-ephemeral-until-first-prompt.md): blank Sessions now render no row anywhere until the first prompt converts them.
