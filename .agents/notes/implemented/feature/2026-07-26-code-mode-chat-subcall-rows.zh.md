...
> 范围：Web chat 视图如何渲染一个 `run_code` 轮次，即 Code Mode UI 栈的客户端侧部分，构建在[宿主侧基础](2026-07-26-code-dispatch-ui-foundation.zh.md)之上（携带完整内容的 `tool/code-dispatch`、必填的结构化 `display` 元数据）。本篇所依托的 slot 模型归 [toolview 溶解](../architecture/2026-07-23-toolview-dissolution.zh.md)所有。
...
- **`run_code` 的呈现**：`code` 行变体（分类器映射 `run_code → code`、标题 `Code`、图标 ui-primitives `CodeXml`）以 presenter 解析后的 `display.name` 作为紧凑摘要，展开后显示程序本身（在 markdown 代码块的填充底色上以等宽字体呈现），并把 `display.description` 作为独立的 `GOAL` 段渲染，而不是把目标塞进名称。持久化通用调用视图对推导名称具有权威性；原始结构化参数与旧扁平 description 仅作为回放回退。
