...
| Package | Role |
|---|---|
| `@monotykamary/dsh-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@monotykamary/dsh-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@monotykamary/dsh-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- id: spill-local
  name: '@monotykamary/dsh-spill-local'

- id: spill-policy
  name: '@monotykamary/dsh-spill-policy'
  config:
...
- `@monotykamary/dsh-output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@monotykamary/dsh-spill` owns saving final text and returning a locator plus retrieval hint.
- `@monotykamary/dsh-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
