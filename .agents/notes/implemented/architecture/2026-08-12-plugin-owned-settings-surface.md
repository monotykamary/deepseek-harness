...
What the gate was not is the boundary its position suggested. Every `settings.*` method sits in `PRIVILEGED_METHODS` (`packages/client/connection`), so a request from an undeclared non-loopback surface, and every cross-origin caller, is refused with 403 before reaching this code — the operator-eligible plane is loopback plus the deployment's trusted authorities (the [privileged-plane trusted surfaces note](2026-08-18-privileged-plane-trusted-surfaces.md)); `role('secret')` fields are structurally stripped from every layer of every response; and the document the plane edits is the user's own `settings.yaml`, which the same settings page offers to open. The writes it did not block were also the consequential ones: `permission` (which can widen the approval preset) and `agent-presets` (which decides what a session mounts) were both already served.
