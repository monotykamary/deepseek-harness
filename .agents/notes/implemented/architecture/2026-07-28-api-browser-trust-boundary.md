...
The trusted-authority list is not frozen at registration: `HostConnectionHandle.addTrustedAuthority` appends a validated bare authority to the per-request-read live list, the entry point the [web remote-surface resolution](../feature/2026-08-17-web-remote-surface-resolution.md) uses to trust derived tailscale and portless names after boot. Two boundaries stay deliberately out of scope: reachability is the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), and authentication for genuinely remote deployments is deferred work recorded in the connection README — the fence is a confused-deputy defense, not an auth layer. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing a header fence does not already cover.
