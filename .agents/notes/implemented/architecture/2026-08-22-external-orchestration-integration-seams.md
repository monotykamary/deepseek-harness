...
The companion `dsh-factory` bundle consumes these mechanisms and the existing Agent, Session, ToolRuntime, Skill, Shell, Attachment, Typert Remote, and preset services. Its SQLite graph and scheduler remain outside this repository because they are one product policy, not a harness primitive. Factory assignments and completion tool calls use the ordinary Session log; the external scheduler does not add a second Agent loop. The app-pinned Web template composes Factory after Fabric and Fovea so a stock Web profile includes the complete control application. Headless remains one-shot and omits Factory because its background scheduler requires a long-lived host.
...
External products can add root applications and repository automation without modifying the Agent loop or replacing shipped UI owners. The custom Web flavor applies that extension path by default through its exact-pinned Factory companion, while Headless retains a bounded one-task lifecycle. Worktree safety has one provider-owned implementation and can acquire non-Git or remote providers later. Conversation remains the default application and retains its state only while selected according to the frame's chain rendering behavior.
