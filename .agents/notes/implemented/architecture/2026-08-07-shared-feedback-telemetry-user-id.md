...
`@monotykamary/dsh-anonymous-user-id` owns `getOrCreateAnonymousUserId()` and the `$DSH_HOME/.anonymous-user-id` storage contract. `session-telemetry-otel` uses the returned id as OpenTelemetry Resource `user.id`; the `/feedback` success acknowledgement reports `Feedback recorded for session {sessionId}` followed by `Anonymous user: {userId}` on a second line; and direct DeepSeek requests carry it as `x-deepseek-harness-user-id` when `requestHeaders.userId` is enabled. Invalid feedback is rejected before resolving the id, and the DeepSeek adapter resolves it only after credentials succeed and only while that header is enabled, so an empty command, a credential failure, or a disabled header never creates `.anonymous-user-id`.
...
- One harness home has one anonymous id shared by feedback acknowledgements, any mounted session telemetry backend, and (when enabled) direct DeepSeek requests.
