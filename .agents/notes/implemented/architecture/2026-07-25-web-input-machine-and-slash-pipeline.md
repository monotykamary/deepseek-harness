...
- Sending unifies in the hub defaultSink: after reference serialization, the effect-owned `ctx.conversation.submissions` registry wraps the ordinary send with the selected Queue/Steer mode and live browser image files, then the unchanged `sendSession` call is the terminal `next()`. Middleware rejection stays inside the submit transaction, so backfill happens only when the live draft is still empty and a user who kept typing is never overwritten. No Draft materialize or attach transaction exists.
...
The state machine's entire behavior is covered by pure-JS unit tests (event sequences in, asserting state and effects, zero browser DOM); the interaction matrix is projection-tested row by row. Registry tests pin order, consume, exactly-once continuation, and disposal, while InputHub orchestration tests prove the wrapper surrounds the real Session prompt sink. This requirement is precisely what forced the pure-core + service-shell layering.
...
- Transactionalized submission (attempt seq + the drift guard) makes the three defect classes — stale-result backwash, session switching, concurrent replay — structurally impossible, pinned by the matrix tests. Middleware can delay or consume an ordinary send; preserving it requires exactly one `next()`, and a Consumer adding model-visible context must log it through its Host capability.
