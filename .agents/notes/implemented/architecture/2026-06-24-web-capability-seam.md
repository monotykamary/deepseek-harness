...
1. `@monotykamary/dsh-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@monotykamary/dsh-web-search-exa`, `@monotykamary/dsh-web-search-perplexity`, `@monotykamary/dsh-web-search-deepseek`, and `@monotykamary/dsh-web-fetch-http`.
3. `@monotykamary/dsh-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@monotykamary/dsh-tool-web  --depends on-->  @monotykamary/dsh-web  <--depends on--  @monotykamary/dsh-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @monotykamary/dsh-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @monotykamary/dsh-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @monotykamary/dsh-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@monotykamary/dsh-web-search-exa"] -->|registerSearchProvider| web["@monotykamary/dsh-web / ctx.web"]
  perplexity["@monotykamary/dsh-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@monotykamary/dsh-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@monotykamary/dsh-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@monotykamary/dsh-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@monotykamary/dsh-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@monotykamary/dsh-tool-web` depends on `@monotykamary/dsh-web`, `@monotykamary/dsh-tools`, `@monotykamary/dsh-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@monotykamary/dsh-web'
...
```yaml
- id: web
  name: '@monotykamary/dsh-web'
  config:
    searchProvider: exa
    fetchProvider: http

- id: web-search-exa
  name: '@monotykamary/dsh-web-search-exa'

- id: web-search-perplexity
  name: '@monotykamary/dsh-web-search-perplexity'

- id: web-search-deepseek
  name: '@monotykamary/dsh-web-search-deepseek'

- id: web-fetch-http
  name: '@monotykamary/dsh-web-fetch-http'
