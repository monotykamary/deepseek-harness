...
`@monotykamary/dsh-invariants` is a product-independent Cordis service plugin that registers `ctx.invariants`. It owns configuration, registration uniqueness, child-fiber lifecycle, and package-attributed failures. It imports no session, agent, scope, or agent-loop package and contains none of their checks.
...
|---|---|---|
| `@monotykamary/dsh-session/invariant` | `@monotykamary/dsh-session` | session sequence, turn/step enclosure, and same-step call/result trace |
| `@monotykamary/dsh-agent/invariant` | `@monotykamary/dsh-agent` | agent-status transitions |
| `@monotykamary/dsh-scope/invariant` | `@monotykamary/dsh-scope` | scoped-event carrier presence and subject consistency |
| `@monotykamary/dsh-agent-loop/invariant` | `@monotykamary/dsh-agent-loop` | model-request reconstruction |
