...
|---|---|---|
| `@monotykamary/cordis` | 4.0.0-rc.7 | 4.0.1 |
| `@monotykamary/cordis-plugin-loader` | 1.0.0-rc.5 | 1.0.1 |
| `@monotykamary/cosmokit` | 1.8.1 | 1.8.2 |
| `@monotykamary/schemastery` | 3.18.0 | 3.18.1 |
| `@monotykamary/cordis-plugin-hmr` | 1.0.15 | 1.0.16 |
| `@monotykamary/cordis-plugin-include` | 1.0.4 | 1.0.5 |
| `@monotykamary/cordis-plugin-timer` | 1.1.2 | 1.1.3 |
| `@monotykamary/cordis-plugin-group` | 1.0.0 | 1.0.1 |
| `@monotykamary/cordis-plugin-logger-console` | 1.0.0 | 1.0.1 |
...
The dsh sequence separates version upload from user-facing dist-tags. `publish --stage` leaves `latest` and `next` unchanged under the `release-candidate` tag; `verify-registry-install` performs a bounded, standard npm global install without legacy peer mode and runs `dsh --version`; `promote` then advances each final tag in dependency-first order and refuses to move a tag to an older semver. A failed install consumes an immutable candidate version but does not expose it through `latest` or `next`.
...
`scripts/check-workspace-constraints.ts` requires the protocol, so a new package cannot reintroduce a hand-written range; the invariant-companion rule requires `workspace:^` for `@monotykamary/dsh-invariants` for the same reason.
...
| `verify-packed-install` | lists every supplied tarball as a top-level file dependency, installs with npm legacy peer mode to verify payload and non-peer runtime closure without reconciling the cyclic peer graph, and drives the executable |
| `verify-registry-install` | installs the exact staged dsh version from npm with ordinary peer resolution, rejects any `ERESOLVE` override, enforces a deadline, and drives the executable |
| `publish` | the three registry states above; dsh uses `release-candidate`, while sequences without a staged executable check publish directly to their final tag |
| `promote` | advances `latest` or `next` only after registry installation passes and refuses semver rollback |
| `process` / `tarball` | the one home for spawning bounded commands and reading a packed tarball, including the entry guard that keeps every script importable; captured commands reserve 64 MiB rather than Node's 1 MiB default |
...
`pack` carries no credentials and runs on every pull request and master push, so a pull request proves the release set still packs. Publication lives in a separate `release-publish.yml` / `release-vendor-publish.yml` workflow that is `workflow_dispatch`-only (so it never appears as a PR check) behind the `npm-publish` environment for human approval. The dsh job stages each entry, performs the exact registry install, and promotes only the verified version; vendor has no installed executable and publishes directly. Pack runs are grouped per ref so concurrent pull requests do not displace each other; the `publish` job carries the global `Release-publish` group, because dist-tags are shared registry state.
...
**Running a local registry before publication.** A local registry would exercise unpublished semver ranges but adds registry software and proxy state to every credential-free pack run. The packed check instead owns payload completeness with supplied `file:` tarballs, while the protected dsh publication stages immutable versions on npm itself and exercises the real resolver and registry metadata before promotion.

**Selecting a subset by entry closure.** Crawling `dependencies` from `@monotykamary/dsh` and `@monotykamary/dsh-web-frontend` yields 156 packages, 61 fewer than the whole set. But this repository's plugins are mounted by name from `cordis.yml` rather than imported: `vendor/cordis-plugin-group` and `vendor/cordis-plugin-logger-console` fall outside the dependency closure while being required at runtime. Selecting by code dependency fails as "the consumer installs it and it will not start", and it would need a standing proof that no mounted package was missed. Under a private scope the extra packages are invisible outside the organization. `python/`, the root `examples/`, `docs/`, and `website/` are not members.
...
A pull request runs the full pack for both sequences without credentials and installs the packed dsh tarballs into a throwaway consumer, where plain Node drives `dsh --version`. That probe proves `files` selected a complete payload and non-peer runtime dependencies install; the protected publication separately proves published peer ranges through an exact standard npm registry install. Neither probe claims interactive behavior.
...
- **A failed registry install consumes the candidate version.** npm versions are immutable, so repairing a staged version that fails ordinary resolution requires another version bump. User-facing tags remain on the last verified release.
- **The registry install is a release-time network check.** It downloads the complete public application graph and has a bounded twenty-minute deadline; pull requests keep the faster credential-free payload check.
