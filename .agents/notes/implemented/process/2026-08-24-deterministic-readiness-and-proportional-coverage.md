...
The hosted macOS deterministic parity run caps Vitest at one worker per project pool. This bound keeps both project-level fork pools within the runner's fixed memory while preserving the complete required unit inventory; a process crash remains blocking rather than being reclassified as an observation.
