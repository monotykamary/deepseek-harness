...
`before: null` 要求展示消费方使用既有的整文件回退。成功更新出现该值时不会发出持久 mutation receipt，因为旧内容的哈希与文本 hunk 均不可用；`operation: 'update'` 判别字段可防止消费方伪造 create receipt。真正的创建仍会发出整文件 receipt。该上限只限制额外获取旧内容的成本，以及上下文内容对是否合格；它不限制调用方持有的替换内容、返回的 `after` 值或回退渲染。
...
部署可以调整额外的覆写基础成本，而不改变读取路由。达到或超过排他上限时，覆写仍会成功，并通过整文件展示回退保持可见，但不再提供上下文 hunk 和 receipt ledger 归属。低于上限时，除调用方的替换内容外，提供方仍可能持有接近 `diffBasisMaxBytes` 的旧文本。对于合格覆写，有上限的描述符读取会增加一次 open/stat/read 序列，同时防止陈旧路径探测把该序列变成无上限分配。
