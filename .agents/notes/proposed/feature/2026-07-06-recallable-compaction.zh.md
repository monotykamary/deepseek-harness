...
新增包 `@monotykamary/dsh-tool-recall`，它只是 `dsh-session` 与 `dsh-compaction` 词汇之上的消费方，注册两个面向模型的工具：
...
- seam JSDoc、压缩能力 seam Agent Note、`architecture.md`，以及生成的工具、配置、持久化与模块图目录都在同一改动中更新；全部预算位于配置中；新源码参与聚合 80% 覆盖率，并具备 HMR（热模块替换）dispose（资源释放）测试。
