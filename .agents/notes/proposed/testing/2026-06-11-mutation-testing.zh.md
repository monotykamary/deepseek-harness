...
聚合 80% 覆盖率门禁（[质量门禁决策](../../implemented/process/2026-06-11-quality-gates.zh.md)）度量代码执行，却不能证明断言会发现错误行为。变异测试衡量覆盖率无法衡量的内容：测试套件是否能*杀死*被刻意注入的缺陷。
...
- 等价变异体（可证明不改变行为的）使用经过评审并附理由的显式排除。
...
- 等价变异体使用经过评审并附理由的显式排除。

## 风险

运行时间：变异测试开销大。如果 PR 范围运行始终过慢，则保持仅每夜运行，依靠变异分数阈值的只升不降机制。
