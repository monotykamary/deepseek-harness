...
The aggregate 80% coverage gate ([the quality-gates decision](../../implemented/process/2026-06-11-quality-gates.md)) measures execution — not whether an assertion would notice wrong behavior. Mutation testing measures what coverage cannot: whether the suite *kills* deliberately injected bugs.
...
- Equivalent mutants (provably behavior-preserving) use explicit reviewed exclusions with reasons.
...
- Equivalent mutants carry explicit reviewed exclusions with reasons.

## Risks

Runtime: mutation testing is expensive. If PR-scoped runs stay too slow, keep them nightly-only and rely on the score ratchet.
