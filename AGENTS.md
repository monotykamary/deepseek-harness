...
packages/    @monotykamary/dsh-<pkg> workspaces at packages/<group>/<pkg>/
...
native/      @monotykamary/node-addon-landlock-run source of record (see native/README.md)
...
- Every npm package is `@monotykamary/dsh-<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@monotykamary/cordis` is a peerDependency (+ dev) of every harness package.
