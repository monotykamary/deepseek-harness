...
| `ctx.worktrees` | `seam` | [`worktree`](../packages/workspace/worktree) | [`worktree-git-local`](../packages/workspace/worktree-git-local) | - | - | Providers own repository discovery, managed checkout allocation, occupancy-aware removal, and bounded sweeping; external orchestration bundles are direct Consumers. |
...
| `ctx.sessionModels` | `core` | `apiproxy` | - | - | - | Shares one resolve, apply, and persistence path between in-process server plugins and the session model RPC methods. |
