...
`bun run test:gui` covers client and host GUI packages without a browser or server; `bun run test:changed` scopes Vitest to dirty packages, and `--coverage` measures only their source. The watch variants iterate on one package. Use these freely; run relevant full checks once.

## Tiers

- **Unit** (`bun run test`): deterministic package, example, and script specs. Prefer edge cases, error paths, event ordering, concurrency races, and contract regressions. Host observations that cannot be synchronized run under `test:observational`; their failures remain visible but cannot reject readiness.
- **Coverage gate** (`bun run test:coverage`): aggregate statements, branches, functions, and lines under `packages/*/*/src` must each reach 80%. Coverage suppression directives are forbidden. Coverage proves execution, not shipped behavior; required behavior checks remain separate.
...
Revision readiness is `true` exactly when the complete required-check set succeeds on that revision; missing, failed, skipped, or cancelled evidence rejects it. Release pack/install workflows report independently of full rehearsals.
