...
`pnpm run test:gui` covers client and host GUI packages without a browser or server; `pnpm run test:changed` scopes Vitest to dirty packages, and `--coverage` measures only their source. The watch variants iterate on one package. Use these freely; run relevant full checks once.

## Tiers

- **Unit** (`pnpm run test`): deterministic package, example, and script specs. Prefer edge cases, error paths, event ordering, concurrency races, and contract regressions. Host observations that cannot be synchronized run under `test:observational`; their failures remain visible but cannot reject readiness.
- **Coverage gate** (`pnpm run test:coverage`): aggregate statements, branches, functions, and lines under `packages/*/*/src` must each reach 80%. Coverage suppression directives are forbidden. Coverage proves execution, not shipped behavior; required behavior checks remain separate.
