...
A **profile** is a named composition stored in the Harness home. A shipped profile records its installation-owned template identity plus user-added bundles, while a custom profile lists only its bundles; both keep the user's own `cordis.patch.yml`. `web` and `headless` ship as templates, each layering the exact external companion versions tested by the running `dsh` installation over the upstream composition.
...
Each declares itself in its own `package.json` under a `dsh` field: `dsh.profile.template` selects an installation-owned template, `dsh.profile.bundles` lists user-managed layers, and `dsh.bundle` points at a bundle's patch file.

[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all. The custom flavor adds `dsh-tool-repair` (schema-guided finalized tool-call repair), `dsh-multiprovider` (provider-neutral account selection, affinity, health, and operator settings), `dsh-fabric` (deterministic compaction, QuickJS code runtime, durable mesh, and live topology), and `dsh-fovea` (foveated repository intelligence) to every shipped template. The long-lived Web template also adds `dsh-factory` (durable dependency graphs, recurring Agent work, checkout lanes, and Triage); Headless stays one-shot and does not start its background scheduler. All five ship as installation dependencies of the `dsh` app.
...
`turn/*`, `step/*`, `user/message`, `assistant/*`, and `tool/*` are durable session events; the rest are live extension points across three domains. The loop tags each root tool execution with its Turn and step; versioned file mutation receipts carry commit order plus SHA-1/SHA-256 transitions and land on the direct `tool/result` or the nested `tool/code-dispatch` that performed the operation. `agent/pre-step`, `agent/request`, `llm/stream`, and the three `tools/*` events are waterfalls, whose listeners must call `next()` to delegate; `agent/turn-stopping` is serial and has no `next()`.
...
| Add a browser terminal Consumer | register a trusted Connection upgrade and contribute to `bottom-panel` or `workbench.surface`; `dsh-terminal-web` and `dsh-client-ui-terminal` are the shipped pair |
