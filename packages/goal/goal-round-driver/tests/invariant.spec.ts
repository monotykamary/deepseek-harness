...
import * as GoalSessionInvariant from '@monotykamary/dsh-goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@monotykamary/dsh-goal-round-driver'
import InvariantRegistry, { InvariantError } from '@monotykamary/dsh-invariants'
import SessionStore, { SessionId, type Session } from '@monotykamary/dsh-session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@monotykamary/dsh-goal-round-driver',
    }))
  })
