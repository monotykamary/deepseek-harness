...
import AgentRegistry, { agentEvents, Inbox } from '@monotykamary/dsh-agent'
import type { Agent } from '@monotykamary/dsh-agent'
import { createUserMessage, HarnessError } from '@monotykamary/dsh-llm'
import SessionStore, { Session, SessionId, type UserMessage } from '@monotykamary/dsh-session'
import GoalService, {
  GoalError,
  GoalId,
  decodeGoalChange,
  foldGoal,
} from '@monotykamary/dsh-goal'
import type { GoalChangeMeta, GoalRef, GoalSnapshotChangeMeta } from '@monotykamary/dsh-goal'
...
function stubAgent(rawId: string, seed?: readonly import('@monotykamary/dsh-session').SessionEvent[]): StubAgent {
