...
| Package | Role |
|---|---|
| `@monotykamary/dsh-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@monotykamary/dsh-web-search-exa` | Search provider: Exa |
| `@monotykamary/dsh-web-search-perplexity` | Search provider: Perplexity |
| `@monotykamary/dsh-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@monotykamary/dsh-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
