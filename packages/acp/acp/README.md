...
| `session/new` | Creates a fresh agent with an absolute primary `cwd`; empty `additionalDirectories` is accepted, while client-provided `mcpServers` are accepted and ignored because DSH owns its tool composition; non-empty `additionalDirectories` rejects. |
