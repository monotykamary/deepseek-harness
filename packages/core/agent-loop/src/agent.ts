...
import type { EpochHeader, RequestContext, Session, SessionId, TurnEndReason, UserMessage } from '@monotykamary/dsh-session'
...
export class ReactLoopAgent implements Agent {
  ...
  private async kick(): Promise<void> {
    try {
      ...
      /* kick owns a running phase until this driver boundary */
  ...
  private async preStep(target: InboxTarget, position: { turn: number; step: number }): Promise<PreparedStep> {
    /* private callers establish the running phase before proposing a step */
  ...
  private async step(assembly: PromptAssembly): Promise<StepEndReason | null> {
    /* private callers establish the running phase before executing a step */
