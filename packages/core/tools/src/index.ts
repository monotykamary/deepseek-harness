 ...
 * execution pipeline.
 * @module @monotykamary/dsh-tools
 */

import { Context, Service } from '@monotykamary/cordis'
import z from '@monotykamary/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@monotykamary/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@monotykamary/dsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@monotykamary/dsh-llm'
...
// lives in its own UI-facing module; re-export it so `@monotykamary/dsh-tools`
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@monotykamary/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @monotykamary/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
