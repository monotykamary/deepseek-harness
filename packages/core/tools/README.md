...
A process config or scoped presentation may set `runCodeLabel: inferred`: only `code` remains required, an explicit non-blank `description` still wins, and otherwise the card title is derived deterministically from literal `tools.*(...)` calls in the logged program (falling back to `Run code`). The default `required` policy remains `{ code, description }`; the selected policy also changes the SDK invocation sentence, so schema and prose stay aligned.
