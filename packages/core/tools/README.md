...
tools:
  ...
  speculation:
    enabled: false          # safe-off Harness default; Fabric opts in
    maxConcurrent: 4
    maxEntries: 64
...
`runCodeLabel` controls the Code Mode transport at the same process scope: `required` exposes `{ code, display }`, where `display` is either `{ name, description? }` or a non-blank name string shorthand; `inferred` makes `display` optional and derives a missing run title from recorded code. In both policies, `display.name` (or the shorthand) is the concise activity title and `display.description` is the optional objective retained for progress surfaces and durable consumers. `inferred` with `mode: native` fails at load. `native` contributes visible tools as function definitions. `code` contributes the reserved `run_code` transport, the generated `tools:sdk` section, and the `tools:code-only` rule stating that only `run_code` may be called directly — which the executor then enforces, resolving a model-direct call naming any other tool to `UNKNOWN_TOOL` before policy runs; `both` contributes both forms and states no such rule, because its native calls do execute. This is the default for agents that declare none of their own — an agent preset selects its own with [`dsh-agent-tool-presentation`](../agent-tool-presentation/README.md). The reserved transport cannot be registered, shadowed, restricted, or removed, and its name is reserved whatever the configured mode, because any agent may select a code mode. Non-native modes require a `ctx.codeRuntime` whose `language` has a registered SDK renderer — TypeScript ships via [`dsh-code-runtime-worker-thread`](../../code-runtime/code-runtime-worker-thread/README.md); a Python renderer is built in and drives any runtime that reports `language: 'python'` (a first-party `dsh-code-runtime-python` backend is delivered separately). A runtime language with no renderer fails prompt assembly loudly, and a `systemPrompt.toolOrder` entry for a tool the mode does not contribute rejects prompt assembly. A `system-prompt/assemble` listener may replace the registry's contributions; its returned assembly is authoritative, so that listener owns preserving a usable Code Mode protocol.

#### Speculative Code Mode

When `speculation.enabled` is true, ToolRuntime observes streamed `run_code` argument deltas and may start complete literal SDK calls before generation finishes. The scanner is TypeScript-only and accepts direct `tools.name({...})` or `tools["name"]({...})` calls whose single argument is statically JSON-literal; Python and dynamic expressions execute only at the natural call point. A definition must explicitly provide `speculate(args, context)`, `risk: "read"`, and `effectKind: "none"`; registration rejects every other combination. The shipped allowlist is therefore capability-based rather than name-based (`read`, `glob`, and `grep` opt in). Harness defaults the feature off; dsh-fabric enables it in composition.

A prefetched value is a take-once promise cache entry scoped by an unforgeable streamed-root owner, exact tool name, and stable arguments; reused provider call IDs cannot cross owners. The final argument object must parse with exactly one top-level `code` string identical to the streamed code, or all hidden work is cancelled. The natural nested call still traverses `tools/pre-execute`, guards, around-dispatch, `tools/post-execute`, finalization, and durable result publication. Hidden work emits none of those effects; an optional `replay(exec)` runs only after policy admits the natural call, and an optional `isFresh(exec)` can reject stale work. Shipped hidden filesystem acquisitions are workspace-confined (including canonical symlink checks); an outside target is left to the policy-authorized natural call. `read` compares provider versions, while `glob` and `grep` rerun the complete search at serve time and reuse a prefetched value only when the canonical result is unchanged. Any earlier real nested body not declared effect-free advances a per-owner mutation epoch and invalidates older entries. Definition replacement, abort, failure, timeout, malformed streams, stream and retained-result byte caps, TTL, concurrency, and entry bounds all fail closed to ordinary execution.

The same six fields are registered as the live `tool-speculation` user-settings namespace. A valid settings commit immediately clears active observers and aborts unserved entries before applying the new bounds; composition config remains the base layer and removing a user field inherits it again.

### Public API

- `ctx.tools.register(definition: ToolDefinition): () => void` Register a trusted typed same-process definition with a mandatory canonical `output` declaration. The layer is the calling context's scope: a plain plugin context registers globally; an agent's `agent.ctx` registers for that agent alone, shadowing a same-named global tool there. Duplicate names within one layer throw; `run_code`, `call`, and `describe` are reserved for Code Mode and reject registration in every mode. Missing or unsupported output declarations and a non-positive or non-finite `timeoutMs` fail at registration. The optional synchronous `finalizeContent` callback is snapshotted when a call starts and may replace only final model-facing content after every pipeline outcome is normalized, including an error discovered while materializing another result field. Disposed with the calling fiber.
- `ctx.tools.presentAs(mode: ToolPresentationMode, options?: ToolPresentationOptions): () => Promise<void>` selects this agent's model-facing presentation, shadowing the `mode` config for that agent alone; it throws from a plain context (a process-wide presentation is the config field) and from a second declaration in the same scope. `options.runCodeLabel` is `required` by default; `inferred` makes `run_code.display` optional and derives a missing card title from the recorded program without changing its arguments. Display metadata separates the concise `name` shown as the card title from the optional `description` objective shown as card content. A code mode also registers that agent's own policy-aware `tools:sdk` section. The catalog is unchanged — `schemas(agent)` still reports the agent's capabilities; only the assembly's tools collapse. Disposed with the calling fiber.
...
- `ToolExecutionInput` — the caller-supplied call description: `{ callId, name, arguments, signal, agent?, parent?, location? }`; `signal` is required and readonly, callers may pass an enclosing execution's opaque token as `parent`, and callers never choose the new execution's own token.
...
- `ToolExecution` — the readonly pipeline view: immutable `{ token, callId, name, arguments, signal, agent?, parent?, location? }`; the registry separately retains and re-fuses the original caller signal. `ToolDispatchExecution` is the `tools/execute`-only view whose required signal is mutable, so a wrapper may replace and restore it but cannot delete it. A nested call's `parent` is a `ToolExecutionToken`, not an execution object.
- `ToolRunContext` — the execution passed to a tool body, extending `ToolExecution` with `deferContext(context)` and `recordFileMutation(mutation)`. Context deferral waits for the final result and never injects immediately. A mutator calls the recorder only after a workspace-file operation commits and supplies complete-content SHA-1/SHA-256 transitions plus textual hunks; the registry adds a version and monotonic per-Session commit order, snapshots the receipt, and preserves it through later result replacement or failure.
- `ToolExecutionResult` — discriminated execution-local outcome. Success is `{ isError:false, value:JsonValue, content, meta?, additionalContexts?, mutations? }`; failure is `{ isError:true, error:{ message, info? }, content, meta?, additionalContexts?, mutations? }` and has no value. Call identity stays on the immutable `ToolExecution`. The registry snapshots, validates, and freezes the canonical value before rendering, then materializes the durable presentation fields before final observation. `ToolFailure.info` carries an internal `{ name, code }` for a `HarnessError`; `additionalContexts` preserves every deferred or post-execute identified `UserMessage` for the loop's post-result FIFO.
...
- **Settlement discipline**: the bridge owns a run-scoped abort that follows the outer signal in and fires when the run settles for any reason, so a budget expiry aborts an in-flight sub-tool instead of orphaning it; the bridge then drains its queue BEFORE returning, so every `tool/code-dispatch` lands inside the open turn. Every binding Promise has a host-side rejection observer while the original Promise is returned unchanged: a runtime that drops an unawaited call cannot turn result discard or queue abandonment into a process-level unhandled rejection, while an awaiting runtime receives the exact failure. A failed run throws `CodeRunFailedError` (`code: 'CODE_RUN_FAILED'`, message = the failure kind + captured logs), which the pipeline converts to a structured `isError` the model self-corrects from. The [binding rejection decision](../../../.agents/notes/implemented/bug-fix/2026-08-26-code-mode-binding-rejection-containment.md) owns this containment.
...
`run_code` takes two required arguments: `code` — the body of an async TypeScript function (erasable syntax only — no `enum` or namespaces; type annotations are advisory, the code runs type-stripped) — and `display`. Prefer `display: { name, description }`: `name` is a brief activity name and `description` is the concise objective; a string is name shorthand. Inside the program:
...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@monotykamary/dsh-tool-call-timeout-policy` wrapper.
