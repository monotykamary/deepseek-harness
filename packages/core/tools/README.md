...
- `ToolRunContext` — the execution passed to a tool body, extending `ToolExecution` with `deferContext(context)` and `recordFileMutation(mutation)`. Context deferral waits for the final result and never injects immediately. A mutator calls the recorder only after a workspace-file operation commits; the registry snapshots the receipt and preserves it through later result replacement or failure.
