...
export const name = 'tool-session-mutations'
...
export function apply(ctx: Context, config: Config): void {
  ctx.tools.register(defineTool({
    ...
    execute(args, exec) {
      ...
      for (const [key, value] of [['after_commit_order', args.after_commit_order], ['commit_order', args.commit_order], ['offset', args.offset]] as const) {
        if (value !== undefined && value < 0) throw new Error(`changes_read ${key} must be nonnegative`)
      }
