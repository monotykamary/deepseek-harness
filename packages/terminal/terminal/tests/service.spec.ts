...
import TerminalSessionService, { TerminalBackendCleanupError, TerminalError, TerminalSessionId } from '@monotykamary/dsh-terminal'
import type {
  TerminalBackend,
...
describe('TerminalSessionService ownership and lifecycle', () => {
  it('publishes only after spawn and fences every operation to the exact owner', async () => {
    ...
    const peer = ctx.terminals.attach(owner, created.sessionId)
    await attachment.write('interactive input')
    await peer.write('peer input')
    await attachment.resize(100, 30)
    expect(attachment.status()).toEqual({ kind: 'running' })
    expect(b.sessions[0]?.interactiveWrites).toEqual(['interactive input', 'peer input'])
    expect(b.sessions[0]?.interactiveResizes).toEqual([{ cols: 100, rows: 30 }])
    attachment.close()
    expect(b.sessions[0]?.attachmentsClosed).toBe(1)
    expect(() => ctx.terminals.startSend(owner, created.sessionId, { text: 'blocked', submit: true }))
      .toThrow(expect.objectContaining({ code: 'INTERACTIVE_ACTIVE' }))
    peer.close()
    expect(b.sessions[0]?.attachmentsClosed).toBe(2)
