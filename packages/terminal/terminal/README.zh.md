...
- 成功的 spawn 会发布一个 `TerminalSessionId`。可选的 `name` 只是所有者本地的显示元数据，绝不代表权限。可选的 `interactive` 意图用于区分人类直接操作的 PTY 与模型 framing 会话，不改变所有权。
- 一个会话最多接受一个活跃的发送操作。读取和信号操作可以观察该发送；在当前操作结算前，另一项发送会失败。
- 一个会话可以公开多个可独立释放的原始查看方。只要任一查看方仍处于附加状态，面向模型的 send 就会被排除；输出分发、有序输入与共享尺寸策略由后端持有。`SerialOperationQueue` 会在调用方 task 中启动 idle 操作、跨异步 provider 保持 FIFO 顺序、隔离单项操作失败，并且仅在 quiescence 时解析 `idle()`。
...
没有直接可见内容。此包不注册提示词或工具；可见 schema 和结果文本由 `@monotykamary/dsh-tool-terminal` 负责。
