...
- One session may expose multiple independently disposable raw viewers. Any attached viewer excludes model-facing sends until every viewer detaches; the backend owns output fan-out, ordered input, and shared-size policy. `SerialOperationQueue` starts an idle operation in its caller's task, retains FIFO order across asynchronous providers, isolates operation failures, and resolves `idle()` only at quiescence.
...
Nothing directly. This package registers no prompt or tool; `@monotykamary/dsh-tool-terminal` owns visible schemas and result text.
