...
import { resolvePwshPath } from '@monotykamary/dsh-pwsh-local'
...
export type ResolvedConfig =
  & Omit<Required<Config>, 'shellDialect' | 'shellPath' | 'shellArgs' | 'interactiveShellPath' | 'interactiveShellArgs'>
  & Pick<Config, 'interactiveShellPath' | 'interactiveShellArgs'>
  & {
    shellDialect: ShellDialect
    shellPath: string
    shellArgs: string[]
...
export const Config: z<Config> = z.object({
  ...
  interactiveShellPath: z.string(),
  interactiveShellArgs: z.array(z.string()),
  ...
  interactiveReplayMaxBytes: z.number().default(1024 * 1024),
...
export function validateConfig(config: Config): asserts config is ResolvedConfig {
  ...
  if (resolved.interactiveShellPath !== undefined && resolved.interactiveShellPath.length === 0) {
