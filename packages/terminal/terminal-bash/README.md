...
An interactive attachment receives a bounded raw replay capped by `interactiveReplayMaxBytes`, then the live PTY byte stream. Its writes bypass send-readiness framing, and resize delegates to the provider-neutral `SubprocessTerminalHandle.resize`; detach removes only that stream tap and leaves the shell alive. Browser and model Consumers can therefore share one backend, although concurrent writes intentionally retain PTY ordering rather than transaction isolation.
