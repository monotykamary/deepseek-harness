...
Each interactive attachment receives a bounded raw replay capped by `interactiveReplayMaxBytes`, then the live PTY byte stream; output fans out to every attached viewer. Input operations from all viewers are serialized. The first viewer owns the PTY dimensions, input promotes its viewer to resize owner and applies that viewer’s latest grid before writing, and owner detach restores the most recently interactive remaining viewer’s grid. Detach removes only that stream tap and leaves the shell alive. Browser and model Consumers can therefore share one backend, although concurrent input has PTY ordering rather than transaction isolation.
