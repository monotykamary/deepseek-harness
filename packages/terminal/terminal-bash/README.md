...
A spawn with `interactive: true` launches the native login shell resolved from `$SHELL` (falling back to the platform default), preserves its startup files and prompt, advertises `TERM=xterm-256color` and `COLORTERM=truecolor`, and publishes without the model shell's controlled readiness prompt. Model sessions retain the configured shell argv and readiness framing.

Each interactive attachment receives a bounded raw replay capped by `interactiveReplayMaxBytes`, then the live PTY byte stream; output fans out to every attached viewer. Raw replay and line-oriented retention use incremental leading-trim deques, so each PTY callback touches only new and discarded chunks instead of rescanning retained history; snapshots concatenate on demand. Input operations from all viewers are serialized. The first viewer owns the PTY dimensions, input promotes its viewer to resize owner and applies that viewer’s latest grid before writing, and owner detach restores the most recently interactive remaining viewer’s grid. Detach removes only that stream tap and leaves the shell alive. An interactive session that stays viewerless for `unattendedExitMs` (default 30 minutes; `0` disables) is closed by the backend, reclaiming orphaned PTY processes. Browser and model Consumers can therefore share one backend, although concurrent input has PTY ordering rather than transaction isolation.
...
The policy owner contributes capability-neutral `sandbox:policy` context. Through `@monotykamary/dsh-tool-terminal` or another PTY consumer, the model may also receive bounded MOTD, send deltas, scrollback pages, readiness reasons, and cleanup errors.
...
- Model-facing retained reads remain line-normalized; raw interactive attachments support alternate-buffer programs, but their replay is bounded and may start after dropped history.
...
- Unattended teardown reclaims viewerless browser sessions after `unattendedExitMs`; a detached long-running command does not extend the window.
