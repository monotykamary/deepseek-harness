...
交互式 attachment 会先收到受 `interactiveReplayMaxBytes` 限制的原始 replay，随后接收实时 PTY 字节流。它的写入绕过 send 就绪 framing，resize 则委托给提供方无关的 `SubprocessTerminalHandle.resize`；detach 只移除该 stream tap，并保留 shell。浏览器与模型 Consumer 因此可以共享同一个后端，但并发写入只保留 PTY 顺序，不提供事务隔离。
