...
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@monotykamary/dsh-llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@monotykamary/dsh-session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@monotykamary/dsh-agent'
import AgentLoop from '@monotykamary/dsh-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@monotykamary/dsh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import ToolRuntime, { defineContentToolFixture } from '@monotykamary/dsh-tools'
import type {
  ToolExecution,
  ToolExecutionToken,
...
} from '@monotykamary/dsh-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    try {
      ...
      const isolated = await import('@monotykamary/dsh-agent-instructions')
