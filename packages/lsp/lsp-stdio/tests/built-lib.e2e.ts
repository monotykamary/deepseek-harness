...
/**
 * Keyless built-artifact smoke: plain Node imports `@monotykamary/dsh-lsp` and
 * `@monotykamary/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@monotykamary/cordis')
      const { default: Lsp } = await import('@monotykamary/dsh-lsp')
      const LspLocal = await import('@monotykamary/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@monotykamary/dsh-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@monotykamary/dsh-subprocess-local')
