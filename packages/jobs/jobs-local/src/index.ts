 ...
 * @module @monotykamary/dsh-jobs-local
 */

import { Context } from '@monotykamary/cordis'
import z from '@monotykamary/schemastery'
import type { Agent } from '@monotykamary/dsh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@monotykamary/dsh-scope'
import type { ScopeLayer } from '@monotykamary/dsh-scope'
import { deadline, timeoutOf } from '@monotykamary/dsh-timeout'
import { JobRegistry, JobId } from '@monotykamary/dsh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@monotykamary/dsh-jobs'
 ...
 * `@monotykamary/dsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @monotykamary/dsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @monotykamary/dsh-agent)')
    }
