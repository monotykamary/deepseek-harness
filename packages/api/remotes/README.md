...
The current Client assembly mounts the Goal Remote, the read-only Host plugin inventory (`pluginInventory/list`), and the Session-authorized workspace file methods (`workspaceFiles/list`, `workspaceFiles/read`). Cordis effect ownership withdraws every contribution when this assembly unloads, while `@monotykamary/dsh-api-gateway/client` owns descriptor validation, traced namespace Services, direct and scoped methods, invocation, and cancellation. The Client entry consumes the shared `TypertClientRemote` interface through Cordis and does not import the concrete Gateway. It re-exports the Gateway Client face's declaration merges type-only, so a consumer reaching the forwarded-event vocabulary through this facade gains no runtime edge to the Gateway implementation.
