...
import type { AuthorizationMethod, AuthorizationPrompt, AuthorizationSession } from '@monotykamary/dsh-authorization'
import { isCredentialKeySegment } from '@monotykamary/dsh-credentials'
...
export function registerPiAiFlows(ctx: Context, auth: PiAiAuthInjection): void {
  for (const providerId of catalogProviderIds()) {
    ...
    /* every id here names an installed provider and every
    ...
    /* every installed catalog id today is a lowercase
