...
import { CallId, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, isContextWindowExceededError, isQuotaExceededError, LlmError, QUOTA_EXCEEDED_CODE } from '@monotykamary/dsh-llm'
import type { FinishReason, StreamChunk, TokenUsage } from '@monotykamary/dsh-llm'
...
function classifyPiAiError(message: string): string {
  ...
  if (/\b(?:network|connection|socket|fetch)\b|\bECONN[A-Z]+\b/i.test(message)
    || /\b(?:other side closed|HTTP2 request did not get a response|WebSocket (?:closed unexpectedly|error))\b/i.test(message)
