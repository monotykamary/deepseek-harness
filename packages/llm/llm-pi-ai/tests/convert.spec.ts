...
import type { AttachmentStore, ImageAttachmentRef, ImageRequestPolicy, RequestImageAttachment } from '@monotykamary/dsh-attachment'
import { createUserMessage, CallId, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, createMessage } from '@monotykamary/dsh-llm'
import type { ContentBlock, StreamChunk } from '@monotykamary/dsh-llm'
...
describe('mapStopReason / mapUsage', () => {
  ...
  it('maps routable HTTP-ish error messages to stable codes', () => {
    ...
    expect(mapStopReason(assistant({
      stopReason: 'error',
      errorMessage: 'Codex error: An error occurred while processing your request. You can retry your request, or contact us through our help center at help.openai.com if the error persists. Please include the request ID b63042a2-a0ec-494d-9a13-47240e4dd9e2 in your message.',
    }))).toMatchObject({ kind: 'error', failure: { code: 'SERVER' } })
