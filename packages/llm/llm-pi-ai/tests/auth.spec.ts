...
import { credentialKey, credentialRef } from '@monotykamary/dsh-credentials'
...
describe('pi credential store over harness records', () => {
  ...
  it('lists only the records this adapter family owns', async () => {
    ...
    // payload is written in a format pi never agreed to.
  ...
  it('treats a provider id outside the record grammar as holding nothing', async () => {
    ...
    // A hand-declared route key is an arbitrary settings dict key, and pi
...
describe('pi ambient auth context', () => {
  beforeEach(() => {
  ...
  it('answers "not set" for a name no reference could ever address', async () => {
    // pi asks about provider-declared names; one outside the reference
