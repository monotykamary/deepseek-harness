...
import AuthorizationService from '@monotykamary/dsh-authorization'
import type { AuthorizationInteraction, AuthorizationNotice, AuthorizationPrompt } from '@monotykamary/dsh-authorization'
...
// The whole of what this module does with pi is run one provider's login
...
/** A context with the record store, the seam, and every pi login flow. */
...
describe('pi login flows', () => {
  it('offers one flow per installed provider, with the methods that provider ships', async () => {
    ...
    // A key-only provider still gets a flow, because pi collects the key
  ...
  it('runs the pi auth type the chosen method names', async () => {
    const ctx = await harness()
  ...
  it('restates every pi login event in the neutral vocabulary', async () => {
    const ctx = await harness()
    const events: AuthEvent[] = [
      ...
      // pi's event union is open; an unrecognised member must still show
  ...
  it('restates every pi prompt, carrying the per-prompt withdrawal signal', async () => {
    const ctx = await harness()
