...
DeepSeek request correlation is separate from app attribution and opt-in. All three fields under `requestHeaders` default off. `userId` sends `x-deepseek-harness-user-id` with the stable anonymous id from [`@monotykamary/dsh-anonymous-user-id`](../../identity/anonymous-user-id/README.md), resolving it lazily so disabled correlation does not create the id file. `sessionId` sends a request's `GenerateOptions.sessionId` as `x-deepseek-harness-session-id`. `compact` sends `x-deepseek-harness-compact: 1` for compaction-purpose requests. Enabled headers go to the resolved `baseURL`, including a configured gateway, and remain outside request bodies and model-visible content. The same fields work through the live `llm-deepseek` settings section.
