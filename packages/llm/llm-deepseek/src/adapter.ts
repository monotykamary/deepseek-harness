...
import { attributionHeaders, contentHasImage, CONTEXT_WINDOW_EXCEEDED_CODE, isContextWindowExceededError, isQuotaExceededError, LlmAdapter, LlmError, offloadRequestImagesWithPolicy, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId } from '@monotykamary/dsh-llm'
import type {
  ContentBlock,
  GenerateOptions,
  LlmModelInfo,
...
import { deadline, idleWatchdog, timeoutOf } from '@monotykamary/dsh-timeout'
import type { AnonymousUserId } from '@monotykamary/dsh-anonymous-user-id'
...
export interface DeepSeekAdapterOptions {
  ...
  /**
   * Resolve the harness-home anonymous id; called only when the user-id
   * request header is enabled, so disabled correlation never creates the id file.
   */
