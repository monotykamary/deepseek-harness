...
同一 seam 的第二个基于库的实现位于 `@monotykamary/dsh-llm-pi-ai`。本包拥有 `deepseek-official` 提供方路由——刻意区别于 pi-ai 的 catalog 名称 `deepseek`，因此同一组合可以并排挂载两条 DeepSeek 路径；而为 `deepseek-official` 本身注册另一个适配器仍会抛出 `LlmError('DUPLICATE_ADAPTER')`。
...
```yaml
- id: llm-deepseek
  name: '@monotykamary/dsh-llm-deepseek'
  config:
    ...
    retryPolicy:             # optional; omission uses normal mode with five retries
      ...
      backoff:
        ...
        maxDelayMs: 10000
        jitterRatio: 0.1
    requestHeaders:          # optional; every field is off by default
      userId: true           # send x-deepseek-harness-user-id
      sessionId: true        # send x-deepseek-harness-session-id
      compact: true          # mark compaction-purpose requests
...
每个 chat 和 Files API 请求都携带 dsh-llm `attributionHeaders()` 的共享归因标头，即用于识别 harness 的必需 `User-Agent` 基线（见 [dsh-llm § 应用归因](../llm/README.zh.md#app-attribution-attributionts)）。直接 DeepSeek 请求与 OpenAI 兼容 gateway 请求都不会获得提供方特定的应用归因标头。

DeepSeek 请求关联独立于应用归因，并且需要显式启用。`requestHeaders` 下三个字段默认全部关闭。`userId` 通过 `x-deepseek-harness-user-id` 发送来自 [`@monotykamary/dsh-anonymous-user-id`](../../identity/anonymous-user-id/README.zh.md) 的稳定匿名 id；系统会惰性解析该 id，因此关闭关联时不会创建 id 文件。`sessionId` 将请求的 `GenerateOptions.sessionId` 作为 `x-deepseek-harness-session-id` 发送。`compact` 为压缩用途请求发送 `x-deepseek-harness-compact: 1`。启用的标头会发送至解析后的 `baseURL`（包括已配置的 gateway），且不会进入请求正文或模型可见内容。同一组字段也可通过实时 `llm-deepseek` settings 分节配置。
