...
import { AttachmentId, ImageVariantId } from '@monotykamary/dsh-attachment'
import type { AttachmentStore, ImageAttachmentRef, RequestImageAttachment } from '@monotykamary/dsh-attachment'
import { createLaunchEnvironmentSnapshot } from '@monotykamary/dsh-launch-environment'
...
import { getOrCreateAnonymousUserId, type AnonymousUserId } from '@monotykamary/dsh-anonymous-user-id'
import { SessionId } from '@monotykamary/dsh-session'
import * as LlmDeepSeek from '@monotykamary/dsh-llm-deepseek'
import { DeepSeekAdapter, resolveAdapterOptions } from '@monotykamary/dsh-llm-deepseek'
...
describe('DeepSeekAdapter against a mock server', () => {
  it('streams a text generation end to end through the assembler', async () => {
    ...
    // App attribution ships on every request; correlation stays opt-in.
  ...
  it('forwards the harness user and session ids when enabled', async () => {
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
  ...
  it('resolves the anonymous id only when its request header is enabled', async () => {
    const server = await mockServer([
      { kind: 'sse', events: textEvents },
      { kind: 'sse', events: textEvents },
    ])
    const resolveUserId = vi.fn(() => TEST_USER_ID)
    const stream = (requestHeaders?: { userId?: boolean }): AsyncIterable<unknown> => new DeepSeekAdapter({
      options: () => resolveAdapterOptions({
        baseURL: server.url,
      ...
      resolveUserId,
    }).stream({ provider: 'deepseek-official', model: 'm', messages: [] })

    await drain(stream())
    expect(resolveUserId).not.toHaveBeenCalled()
    expect(server.headers[0]).not.toHaveProperty('x-deepseek-harness-user-id')

    await drain(stream({ userId: true }))
...
describe('plugin registration and config', () => {
  ...
  it('resolves connection facts and the credential exactly once per stream call', async () => {
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    const options = vi.fn(() => resolveAdapterOptions({
      baseURL: server.url,
