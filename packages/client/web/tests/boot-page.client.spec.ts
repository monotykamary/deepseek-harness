...
describe('BootPage', () => {
  ...
  it('lists failed entries', () => {
    const { el, page } = mount()
    page.setState('@monotykamary/dsh-client-ui-layout', 'failed')
    page.setState('ok', 'active')
    page.setState('@monotykamary/dsh-client-ui-tool', 'failed')
    expect(el.textContent).toContain('@monotykamary/dsh-client-ui-layout')
    expect(el.textContent).toContain('@monotykamary/dsh-client-ui-tool')
