 ...
 */
import type { ClientContext, SessionId, SnapshotStore } from '@monotykamary/dsh-client-runtime/client'
import type { Branded } from '@monotykamary/dsh-brand'
import type {
  ArbitrateKey, ArbitrateOutcome, CommandClaim, ConsumeTokenRequest, PickOutcome,
  ReferenceInsert, SubmitOutcome, TokenSpan,
} from '@monotykamary/dsh-client-ui-input-trigger/client'
...
/** Registry face other plugins reach through `ctx.conversation.blocks`. */
export interface ComposerBlocks {
  /** @param sessionId - affected session. @param block - block to set or undefined to clear. */
  ...
  /** @param sessionId - session whose retained store is dropped. */
...
export interface ComposerSubmissionMiddleware {
  ...
  /** @param request - serialized prompt. @param next - continue exactly once. @returns admission outcome. */
...
export interface ComposerSubmissions {
  /** @param middleware - wrapper to register. @returns disposer for that registration. */
