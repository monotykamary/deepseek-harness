...
function styleInjectionModule(
  ...
  source.push(classMap === undefined ? 'export default {};' : `export default ${JSON.stringify(classMap)};`)
  return source.join('\n')
}
 ...
 * Everything else under @monotykamary/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@monotykamary\/dsh-(host-apiproxy|file-reference|session|llm|tools|brand)(\/|$)/
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @monotykamary value import is a build error — a
