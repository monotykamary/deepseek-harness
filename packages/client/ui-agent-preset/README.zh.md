...
`agentPreset.read`、`copy`、`openDocument` 与 `remove` 被限定给运营者可达面（见 [`dsh-client-connection`](../connection/README.md)）——loopback 或部署可信权威，或配置了身份权威时的运营者 Bearer 令牌：组装指明了一个会话所运行的插件，因此读取它是侦察，其余几个则管理名单并驱动宿主桌面。`agentPreset.list` 不在其中——它携带 id、信任级别与两个不含路径的能力标志，而局域网客户端的选择器需要它。
