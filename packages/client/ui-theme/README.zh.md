...
`src/styles/` 下有五张样式表，全部由 web 壳的 `base.css` 导入：`base.css`、`design-platform.css`、`scrollbar.css`、`gradient-shadow-text.css` 与 `shiki.css`。`scrollbar.css` 是 `--dsw-alias-scrollbar-*` token 的唯一消费方，必须排在声明这些 token 的 `design-platform.css` 之后。`design-platform.css` 中改编自 T3 的角色按消费方拆分：`--dsw-specific-command-palette*` 为 [ui-command-palette](../ui-command-palette/README.md)提供对话框半透明、分组行、键帽、遮罩与页脚层级；`--dsw-specific-sidebar-{control-fill,icon,row-hover,row-active}` 为 [ui-sidebar](../ui-sidebar/README.md)和 [ui-workspace](../ui-workspace/README.md)提供弱化控件与仅在交互时出现的卡片表面；`--dsw-specific-conversation-*` 则为 [ui-conversation](../ui-conversation/README.md)提供会话作用域画布、消息层级、分隔线与编辑器玻璃效果。生成的第三方声明保留已评审的 T3 Code 修订版与完整 MIT 文本。
