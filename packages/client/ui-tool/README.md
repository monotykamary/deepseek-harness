...
`ToolCallTree` receives one root `ToolCallBlock` that already contains recursive `subCalls`, selection state, the session `cwd`, and callbacks for opening files and inspecting calls. It recursively walks the standard call blocks and sends the root and children at every depth through the same atomic dispatch path, without subscribing to a separate parent-to-children map. An expanded row's **Inspect** action invokes the owner callback; the shipped Chat owner selects that call and opens [`ui-workbench`](../ui-workbench/README.md) rather than making Tool presentation depend on panel state.
...
The package also fills the `conversation.details.tool` child declared by the workbench Inspect entry with `ToolDetails`. The row and details renderers share the same pure card models for `terminal`, `read`, `diff`, `search`, and `web` render intents. Unknown intent tags and malformed wire card data fall back to flattened Tool result text.
