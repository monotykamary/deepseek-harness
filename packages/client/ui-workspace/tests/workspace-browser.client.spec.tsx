...
function workspaceRow(label: string): HTMLElement {
  const row = screen.getAllByRole('treeitem').find(candidate =>
    candidate.hasAttribute('aria-expanded') && (candidate.textContent?.includes(label) ?? false))
  if (row === undefined) throw new Error(`Workspace row not found: ${label}`)
  return row
}

function clickWorkspace(label: string): void {
...
describe('WorkspaceBrowser', () => {
  ...
  it('defaults to one updated list and filters it through the All Folders selector', () => {
    localStorage.clear()
    ...
    expect(b.store.getSnapshot()).toMatchObject({ groupBy: 'flat', orderBy: 'updated', workspaceScope: null })
    expect(screen.queryByRole('button', { name: '视图选项' })).toBeNull()
    expect(screen.getByText('所有文件夹')).toBeTruthy()
    expect(screen.getByText('alpha-s')).toBeTruthy()
    expect(screen.getByText('beta-s')).toBeTruthy()

    const filter = screen.getByRole('button', { name: '按工作区筛选会话' })
    expect(filter.querySelector('.lucide-chevron-down')).toBeTruthy()
    fireEvent.click(filter)
    expect(screen.getByRole('menuitem', { name: '所有文件夹' })).toBeTruthy()
    expect(screen.getByRole('menuitem', { name: 'alpha' })).toBeTruthy()
    expect(screen.getByRole('menuitem', { name: 'beta' })).toBeTruthy()
    fireEvent.click(screen.getByRole('menuitem', { name: 'alpha' }))
    expect(b.store.getSnapshot().workspaceScope).toBe(wid('alpha'))
    expect(screen.getByText('alpha-s')).toBeTruthy()
    expect(screen.queryByText('beta-s')).toBeNull()

    fireEvent.click(filter)
    fireEvent.click(screen.getByRole('menuitem', { name: '工作区“alpha”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '重命名' }))
    expect(screen.getByRole('dialog', { name: '重命名工作区' })).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '取消' }))

    fireEvent.click(filter)
    fireEvent.click(screen.getByRole('menuitem', { name: '所有文件夹' }))
    expect(b.store.getSnapshot().workspaceScope).toBeNull()
    expect(screen.getByText('beta-s')).toBeTruthy()
  ...
  it('shows local metadata matches immediately, then clears back to the grouped tree', async () => {
    ...
    try {
      ...
      const input = screen.getByPlaceholderText<HTMLInputElement>('搜索')
  ...
  it('keeps the Search row mounted while focus and query change', () => {
    mount()
    const input = screen.getByRole<HTMLInputElement>('searchbox', { name: '搜索会话' })
  ...
  it('rail state renders icon controls and focuses persistent Search after expansion', () => {
    vi.useFakeTimers()
    try {
      const expandSidebar = vi.fn()
      const b = mount({ wide: false, expandSidebar })
      expect(screen.queryByText('所有工作区')).toBeNull()
  ...
  it('hides the add button when no directory-flow occupant is composed', () => {
    ...
    expect(workspaceRow('alpha')).toBeTruthy()
  })
  ...
  it('uses the full expanded Workspace section when resolving a Workspace drop half', () => {
    ...
    let targetSection = workspaceRow('beta').parentElement as HTMLElement
  ...
  it('draws the first Workspace insertion boundary on the scroll container', () => {
    ...
    const source = workspaceRow('beta')
    let firstSection = workspaceRow('alpha').parentElement as HTMLElement
  ...
  it('accepts a document-level drop and commits the last Workspace marker on drag end', () => {
    ...
    const source = screen.getByText('tail').closest('[role="treeitem"]') as HTMLElement
    let target = workspaceRow('beta').parentElement as HTMLElement
