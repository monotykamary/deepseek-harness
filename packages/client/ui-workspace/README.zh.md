...
- **不受信任的浏览器使用发货 auto-settle 值**：设置平面不会向不受信任的权威公开，因此它在进程内使用启用／三天；可信表面与 loopback 一样读取持久化的 `ui-workspace` namespace。
