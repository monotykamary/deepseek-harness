...
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@monotykamary/dsh-tools'
...
import ApprovalService from '@monotykamary/dsh-user-approval'
import type { ApprovalOutcome } from '@monotykamary/dsh-user-approval'
import { ShellExecutor } from '@monotykamary/dsh-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@monotykamary/dsh-shell'
...
import type { ShellProcessRead } from '@monotykamary/dsh-shell'
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @monotykamary/dsh-jobs and @monotykamary/dsh-tool-jobs')
  })
