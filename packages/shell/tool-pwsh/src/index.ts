 ...
 * `@monotykamary/dsh-pwsh-local`) backs `ctx.shell`; the tool contract is
 ...
 * exit-status parse from `@monotykamary/dsh-shell`.
 *
 * @module @monotykamary/dsh-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@monotykamary/cordis'
import z from '@monotykamary/schemastery'
import { defineTool, TOOL_ABORTED } from '@monotykamary/dsh-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@monotykamary/dsh-tools'
...
import type { SandboxExecutionPolicy, SandboxMode } from '@monotykamary/dsh-sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@monotykamary/dsh-sandbox'
import type { SandboxPolicyService } from '@monotykamary/dsh-sandbox-policy'
import type { ShellRunResult } from '@monotykamary/dsh-shell'
import { parseExitStatus } from '@monotykamary/dsh-shell'
import { processOutcome } from './background.ts'
import { renderPwshProcessRead, renderPwshResult } from './render.ts'
import type { RenderablePwshResult } from './render.ts'

declare module '@monotykamary/dsh-jobs' {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: PwshToolArgs, exec) {
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @monotykamary/dsh-jobs and @monotykamary/dsh-tool-jobs')
        }
