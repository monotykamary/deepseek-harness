...
| Package | Role |
|---|---|
| `@monotykamary/dsh-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@monotykamary/dsh-bash-local` | Service Provider: local subprocesses |
| `@monotykamary/dsh-bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@monotykamary/dsh-tool-bash` | the model-facing tool schemas over `ctx.shell` |
