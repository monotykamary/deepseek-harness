...
npm scope 为 `@monotykamary/dsh-*`；Cordis `Service` 子类和函数插件通过 `ctx.effect()`、`ctx.on()` 或 `ctx.waterfall()` 注册。规则见[包](AGENTS.md)与[根规则](../AGENTS.md#conventions)。

## 层级结构

包按组置于 `packages/<group>/<pkg>/`；包名仍为 `@monotykamary/dsh-<pkg>`。**组 README 负责包／ctx 键映射。**
...
| [`distribution/`](distribution/README.zh.md) | 已安装闭包清单、更新检查与渠道感知安装 | 产品：稳定 API |
...
| [`workspace/`](workspace/README.zh.md) | Workspace 实体与仓库 worktree provider | 产品：稳定 API |
