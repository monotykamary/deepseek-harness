 ...
 * @module @monotykamary/dsh-agent-presets/authoring
 */
...
import { expandHomePath } from '@monotykamary/dsh-home-paths'
...
async function tightenModes(dir: string): Promise<void> {
  ...
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      /* Windows exposes no POSIX owner-execute bit; the POSIX lane covers both file modes. */
