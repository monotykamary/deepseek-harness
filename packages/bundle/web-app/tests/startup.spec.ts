...
import { internals, provideCmdline } from '@monotykamary/dsh-cmdline'
...
`)
  writeFileSync(join(dir, 'cordis.yml'), [
    ...
    '    busyPort: !!js ctx.webStartup.busyPort',
...
describe('web command-line provider', () => {
  ...
  it('leaves deployment values to each consumer when flags omit them', async () => {
    const { values, observed } = await bootProvider([])
    expect(values).toEqual({ openBrowser: true, busyPort: 'random', trustedHosts: [], tailnet: false, portless: false })
