...
import { createLaunchEnvironmentSnapshot, DSH_LAUNCH_ENVIRONMENT_KEY } from '@monotykamary/dsh-launch-environment'
import SystemPrompt from '@monotykamary/dsh-system-prompt'
import type { WebServer } from '@monotykamary/dsh-host-webserver'
...
describe('web-app runtime glue', () => {
  ...
  it('publishes no readiness side effect when printing and browser opening are disabled', async () => {
    ...
    internals.openBrowser = openBrowser
    apply(ctx, new Config({
      openBrowser: false, printUrl: false, surfaceContext: true, trustedHosts: [], tailnet: false, portless: false,
    }))
    await ctx.plugin(SystemPrompt, { persona: '' })
    await new Promise(resolve => setTimeout(resolve, 0))
  ...
  it('prints the loopback-only URL line when no LAN snapshot exists', async () => {
    ...
    ctx.provide('webServer', fakeHttpServer().server)
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(ctx, new Config({ openBrowser: false, printUrl: true, surfaceContext: true, trustedHosts: [] , tailnet: false, portless: false }))
    await new Promise(resolve => setTimeout(resolve, 0))
  ...
  it.each([
    ...
    const openBrowser = vi.fn(async () => {})
    internals.openBrowser = openBrowser
    apply(ctx, new Config({ openBrowser: true, printUrl: true, surfaceContext: false, trustedHosts: [] , tailnet: false, portless: false }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).toHaveBeenCalledWith('dsh web: http://127.0.0.1:4567')
    expect(openBrowser).not.toHaveBeenCalled()
    await ctx.fiber.dispose()
  ...
  it('defers readiness publication until Loader settlement and drops it on failure or teardown', async () => {
    ...
    apply(torn, new Config({ openBrowser: true, printUrl: true, surfaceContext: true, trustedHosts: [] , tailnet: false, portless: false }))
  ...
  it('settles enabled surfaces: derived authorities reach the fence and the URL line announces them', async () => {
    ...
    provideLoader(ctx)
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(ctx, new Config({ openBrowser: false, printUrl: true, surfaceContext: false, trustedHosts: [], tailnet: true, portless: true }))
    await new Promise(resolve => setTimeout(resolve, 0))
  ...
  it('adds surface authorities even when the URL line is silenced', async () => {
    ...
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(ctx, new Config({ openBrowser: false, printUrl: false, surfaceContext: false, trustedHosts: [], tailnet: true, portless: false }))
    await new Promise(resolve => setTimeout(resolve, 0))
  ...
  it('drops a derived surface whose authority the fence refuses and warns instead of widening trust', async () => {
    ...
    provideLoader(ctx)
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(ctx, new Config({ openBrowser: false, printUrl: true, surfaceContext: false, trustedHosts: [], tailnet: true, portless: false }))
    await new Promise(resolve => setTimeout(resolve, 0))
