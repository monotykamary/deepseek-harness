...
ACP automation server app: the default agent spine, client-created agents through [`@monotykamary/dsh-acp`](../../acp/acp/README.md), JSONL persistence, and semantic checkpointing behind one JSON-RPC stdio bin. Programmatic clients create fresh sessions; this package mounts no human UI.

## Composition

| Plugin | Role |
|---|---|
| `@monotykamary/dsh-agent-spine-demo` | Providerless agent spine with no pre-created agents; `session/new` creates each agent. |
| `@monotykamary/dsh-session-persistence-jsonl` | Durable session logs used by checkpointing, observability, and snapshot replay. |
| `@monotykamary/dsh-session-checkpoint-policy` | Durability barriers before model calls and top-level tool effects, plus completed-step checkpoints. |
| `@monotykamary/dsh-session-query-sqlite` | Derived exact/FTS session-query service, opened before the ACP transport so leaf consumers are ready for the first model request. |
| `@monotykamary/dsh-acp` | Automation-only ACP transport over stdin/stdout. |
...
| `tools` | `{ mode: 'native', runCodeLabel: 'required' }` | Native, Code Mode, or combined transport plus the Code Mode run-label policy. |
