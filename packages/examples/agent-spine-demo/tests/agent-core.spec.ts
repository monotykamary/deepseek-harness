...
import { renderPrompt, TOOL_ORDER_REST } from '@monotykamary/dsh-system-prompt'
...
/**
 * Unit coverage for the @monotykamary/dsh-agent-spine-demo bundle: mounting it brings
...
describe('dsh-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@monotykamary/dsh-agent$'] },
      { package_blocklist: ['^@monotykamary/dsh-session$'] },
    ]) {
