...
```
@monotykamary/cordis-plugin-timer  timer service (writes nothing to stdout)
@monotykamary/dsh-llm              abstract LLM service + content-block vocabulary
@monotykamary/dsh-session          event-sourced session log + store
@monotykamary/dsh-session-title    log-backed title service + deterministic fallback
@monotykamary/dsh-system-prompt    prompt-section + tool-schema assembly
@monotykamary/dsh-tools            registry + guarded pre/around/post/final-result pipeline
@monotykamary/dsh-skill            skill provider registry
@monotykamary/dsh-skill-filesystem      local filesystem skill provider
@monotykamary/dsh-agent            agent registry + initiator scope + agent/* events
@monotykamary/dsh-goal             optional persisted same-session goal domain
@monotykamary/dsh-tool-goal        optional model-facing goal controls
@monotykamary/dsh-goal-round-driver     optional same-session goal-round driver
@monotykamary/dsh-llm-retry        provider-routed request retry policy
@monotykamary/dsh-jobs-local      generic background-job registry
@monotykamary/dsh-invariants       configurable invariant registry service
@monotykamary/dsh-session/invariant
@monotykamary/dsh-agent/invariant
@monotykamary/dsh-scope/invariant
@monotykamary/dsh-agent-loop/invariant
                                  package-owned relational checks
@monotykamary/dsh-tool-bash        the model-facing bash schema (unless toolBash=false)
@monotykamary/dsh-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@monotykamary/dsh-tool-skill       session-prefix skill catalog + model-facing loader schema
@monotykamary/dsh-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@monotykamary/dsh-agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@monotykamary/dsh-agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@monotykamary/dsh-'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.zh.md)。
