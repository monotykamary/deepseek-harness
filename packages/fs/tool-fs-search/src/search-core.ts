...
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@monotykamary/dsh-subprocess'
import type { SaveTextSpill, SpillRef } from '@monotykamary/dsh-spill'
 ...
 * `@monotykamary/dsh-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@monotykamary/dsh-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@monotykamary/dsh-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@monotykamary/dsh-tool-fs-search/presentation} `grepSearchMeta`)
