 ...
 * remain session-free.
 *
 * @module @monotykamary/dsh-sandbox-policy
 */
...
import { canonicalPath, type SandboxExecutionPolicy, type SandboxMode } from '@monotykamary/dsh-sandbox'
...
function renderPolicyContext(policy: SandboxExecutionPolicy): string {
  switch (policy.mode) {
    ...
    /* SandboxMode is a typed same-process closed union; this branch is only the static exhaustiveness guard. */
    default: {
