 ...
 * @module @monotykamary/dsh-sandbox-local
 */
...
} from '@monotykamary/node-addon-landlock-run'
import { Context } from '@monotykamary/cordis'
import z from '@monotykamary/schemastery'
import { assertNever } from '@monotykamary/dsh-llm'
import { SandboxProvider, SandboxUnavailableError } from '@monotykamary/dsh-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@monotykamary/dsh-sandbox'
import type { SessionId } from '@monotykamary/dsh-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@monotykamary/dsh-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@monotykamary/dsh-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@monotykamary/dsh-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@monotykamary/dsh-sandbox-windows-acl/src/runner.ts'))
