...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      {
        signature: 'presentAs(mode: ToolPresentationMode, options: ToolPresentationOptions = {}): () => Promise<void>',
        ...
        parameters: [{ name: 'mode', description: 'the presentation the covered agents\' models see.' }, { name: 'options', description: 'Code Mode behavior attached to the same scoped declaration.' }],
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.agent - the newly registered agent with its live session and completed setup. Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/disposed',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.agent - the exact agent removed from the registry. Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/error',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.error - the failure, verbatim. Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.turn - the owning turn. Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.message - the discarded message. Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.message - the inserted message. Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/pre-step',
    mode: 'waterfall',
    ...
    parameters: [{ name: 'payload', description: '.signal - the current turn\'s cancellation signal. Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/request',
    mode: 'waterfall',
    ...
    parameters: [{ name: 'payload', description: '.signal - the current turn\'s explicit abort signal. Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.signal - the turn abort signal. Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.source - why the session started (fresh startup, resume, …). Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/status',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.status - the status just entered (the transition\'s destination). Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Ask composed answerers for one decision. Return an outcome to claim the request or call `next()`; failure yields the fail-closed default. Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.',
    parameters: [{ name: 'req', description: 'the pending decision (agent, tool identity, reason, signal).' }],
  },
  {
  ...
  },
  {
    name: 'goal/changed',
    mode: 'emit',
    signature: '\'goal/changed\'(this: import(\'@monotykamary/dsh-scope\').Scoped<Agent>, payload: { agent: Agent; change: GoalChanged }): void',
    summary: 'Goal mutation accepted by one live agent.',
    description: 'Goal mutation accepted by one live agent. The matching `goal/change` session event has already committed. Listener failures are contained. Scope-filtered dispatch (`@monotykamary/dsh-scope`): agent-scoped listeners receive only that agent.',
  ...
  },
  {
    ...
    description: 'Emitted once when an announced session leaves the store, including publication rollback, but never for an entry whose creation announcement did not begin. Listener failures are logged and contained. Scope-filtered dispatch (`@monotykamary/dsh-scope`) reuses the owner scope.',
  ...
  },
  {
    ...
    description: 'Awaited parallel durability checkpoint: every listener runs and the caller awaits all of them, with no waterfall veto. Scope-filtered dispatch (`@monotykamary/dsh-scope`) reuses the session\'s owner scope.',
    parameters: [{ name: 'session', description: 'the session whose buffered events must reach durable storage.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Observe the frozen, lossless-JSON final outcome. Listener failures are contained. Scope-filtered dispatch (`@monotykamary/dsh-scope`): keyed by `exec.agent`.',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'RunCodeLabelMode',
    declaration: 'export type RunCodeLabelMode = \'required\' | \'inferred\';',
  },
  {
  ...
  {
    name: 'TerminalInteractiveAttachment',
    declaration: 'export interface TerminalInteractiveAttachment {\n    readonly output: Readable;\n    readonly replayTruncated: boolean;\n    write(data: string): Promise<void>;\n    resize(cols: number, rows: number): Promise<void>;\n    status(): TerminalSessionStatus;\n    close(): void;\n}',
  },
  {
  ...
  {
    name: 'ToolEffectKind',
    declaration: 'export type ToolEffectKind = \'none\' | \'state\' | \'workspace\' | \'external\';',
  },
  {
    name: 'ToolErrorInfo',
  ...
  {
    name: 'ToolPresentationOptions',
    declaration: 'export interface ToolPresentationOptions {\n    readonly runCodeLabel?: RunCodeLabelMode;\n}',
  },
  {
  ...
  {
    name: 'ToolRisk',
    declaration: 'export type ToolRisk = \'read\' | \'write\' | \'execute\' | \'network\';',
  },
  {
    name: 'ToolRunContext',
  ...
  {
    name: 'ToolSpeculationContext',
    declaration: 'export interface ToolSpeculationContext {\n    readonly rootCallId: CallId;\n    readonly agent?: Agent;\n    readonly location?: ToolExecutionLocation;\n    readonly signal: AbortSignal;\n}',
  },
  {
    name: 'ToolSpeculationObserver',
    declaration: 'export interface ToolSpeculationObserver {\n    push(argumentsDelta: string): void;\n    finish(argumentsJson: string): void;\n    cancel(): void;\n}',
  },
  {
    name: 'ToolSpeculationResult',
    declaration: 'export interface ToolSpeculationResult<T = JsonValue> {\n    readonly value: T;\n    readonly isFresh?: (exec: ToolRunContext) => boolean | Promise<boolean>;\n    readonly replay?: (exec: ToolRunContext) => void | Promise<void>;\n}',
  },
  {
    name: 'ToolSpeculationStats',
    declaration: 'export interface ToolSpeculationStats {\n    launched: number;\n    served: number;\n    epochInvalidated: number;\n    freshnessInvalidated: number;\n    definitionInvalidated: number;\n    failed: number;\n    wasted: number;\n    skipped: number;\n}',
  },
  {
