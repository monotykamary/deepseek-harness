...
import SessionStore, { SessionId, type SessionEvent } from '@monotykamary/dsh-session'
import SessionPersistenceSqlite, {
  DEFAULT_BUSY_TIMEOUT_MS,
  SCHEMA_VERSION,
} from '@monotykamary/dsh-session-persistence-sqlite'
import {
...
describe('rowToMeta', () => {
  ...
  it('rejects fractional stored creation metadata', () => {
    expect(() => decodeSessionRow({
      ...
      agent_preset: null,
      owner: null,
    })).toThrow('stored created_at must be a safe integer')
  })
...
describe('SessionPersistenceSqlite physical packing', () => {
  it('loads from cordis.yml and packs through the assembled service', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@monotykamary/dsh-session-persistence-sqlite'",
      '  config:',
    ...
    ctx.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@monotykamary/dsh-session') return SessionStore
        if (specifier === '@monotykamary/dsh-session-persistence-sqlite') {
