...
- stdio 发送按服务器串行化：同一时刻至多一个写入等待子进程 stdin 背压。MCP SDK 否则会为每个并发发送堆叠一个 `drain` 监听器，并行的工具扇出会突破 Node 的监听器上限，并对子进程管道触发 MaxListenersExceededWarning。JSON-RPC over stdio 本身有序，因此串行化不改变线路语义。
