...
import type { TypertContribution } from '@monotykamary/dsh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@monotykamary/dsh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 60_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@monotykamary/dsh-tools'],
    }).analyze()
    ...
    const artifact = new FaceModelEmitter(host).emit('@monotykamary/dsh-tools')
    ...
    const record = ctx.typert.getPackage('@monotykamary/dsh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@monotykamary/dsh-tools', 'host')).toBeUndefined()
  })
})
