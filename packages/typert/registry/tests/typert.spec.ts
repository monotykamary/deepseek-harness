...
} from '@monotykamary/dsh-typert-registry'
import type {
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@monotykamary/dsh-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@monotykamary/dsh-tools', 'ToolInput')).toBe('@monotykamary/dsh-tools#ToolInput')
    expect(typertPackageKey('@monotykamary/dsh-tools', 'host')).toBe('@monotykamary/dsh-tools#host')
    expect(ctx.typert.get('@monotykamary/dsh-tools#ToolInput')).toMatchObject({
      package: '@monotykamary/dsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@monotykamary/dsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@monotykamary/dsh-tools', 'host')).toMatchObject({
      key: '@monotykamary/dsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@monotykamary/dsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@monotykamary/dsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@monotykamary/dsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@monotykamary/dsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@monotykamary/dsh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@monotykamary/dsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@monotykamary/dsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@monotykamary/dsh-tools#ToolInput')).not.toBe(projected)
  })
