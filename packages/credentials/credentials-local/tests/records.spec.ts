...
import { credentialKey, credentialKeyScope, credentialRef, parseCredentialKey } from '@monotykamary/dsh-credentials'
import type { CredentialKey, CredentialRecord } from '@monotykamary/dsh-credentials'
...
const CODEX = credentialKey('llm-pi', 'openai-codex')
const BEDROCK = credentialKey('llm-pi', 'amazon-bedrock')
...
describe('credential keys', () => {
  it('rejects a segment that is not a lowercase hyphenated identifier', () => {
    expect(() => credentialKey('llm-pi', 'OpenAI')).toThrow(/credential key segment/)
...
describe('record storage', () => {
  ...
  it('reads every record shape back off disk', async () => {
    ...
    expect(await ctx.credentials.readRecord(BEDROCK)).toEqual({ kind: 'api-key', env: { AWS_PROFILE: 'prod' } })
    expect(await ctx.credentials.readRecord(credentialKey('llm-pi', 'azure')))
      .toEqual({ kind: 'api-key', key: 'sk-azure' })
    expect(await ctx.credentials.readRecord(credentialKey('llm-pi', 'both')))
      .toEqual({ kind: 'api-key', key: 'sk-both', env: { REGION: 'eu' } })
    expect(await ctx.credentials.readRecord(credentialKey('llm-pi', 'ambient'))).toEqual({ kind: 'api-key' })
  })
  ...
  it('keeps two owners of the same provider id apart', async () => {
    const dir = await tempDir()
    const ctx = await boot({ path: join(dir, '.credentials.yaml'), watch: false })
    await put(ctx, CODEX, { kind: 'grant', payload: { owner: 'pi' } })
    await put(ctx, OTHER_OWNER, { kind: 'grant', payload: { owner: 'kimi' } })

    expect(await ctx.credentials.readRecord(CODEX)).toEqual({ kind: 'grant', payload: { owner: 'pi' } })
  ...
  it('refuses a payload this document could not read back', async () => {
    ...
    for (const payload of [{ at: new Date(0) }, { size: 1n }, { run: () => undefined }, { ratio: Number.NaN }]) {
      await expect(put(ctx, CODEX, { kind: 'grant', payload }))
        .rejects.toThrow(/record "llm-pi\/openai-codex" payload/)
    }
    expect(await ctx.credentials.readRecord(CODEX)).toBeUndefined()
